...
describe('prim mod fixture corpus', () => {
  ...
  for (let i = 0, { length } = fixtures; i < length; i += 1) {
    ...
    test(fixture.name, async () => {
      ...
      try {
        assertFixtureValidation(fixture.expected, result)
        assertFixtureFiles(fixture.expected, result, targetRoot)
      } finally {
        cleanup()
      }
    })
...
function assertFixtureValidation(
  expected: Expected,
  result: Awaited<ReturnType<typeof applyCodemod>>,
): void {
  if (expected.ok) {
...
function assertFixtureFiles(
  ...
  for (const [relPath, fileExpected] of Object.entries(expectedFiles)) {
    ...
    assertFixtureContent(absPath, relPath, fileExpected)
  }
}
