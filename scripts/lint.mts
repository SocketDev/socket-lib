...
import { getChangedFilesSync } from '@socketsecurity/lib-stable/git'
import { getStagedFilesSync } from '@socketsecurity/lib-stable/git'
...
export function getOxfmtExcludePatterns(): string[] {
  try {
    ...
    const ignorePatterns = oxfmtConfig['ignorePatterns'] ?? []
...
export function filterLintableFiles(files: string[]): string[] {
  ...
  const formatterExcludePatterns = getOxfmtExcludePatterns()
  ...
  return files.filter(file => {
    ...
    // Filter out files excluded by formatter config
    if (isExcludedByFormatter(file, formatterExcludePatterns)) {
      return false
    }
