 ...
 *   regular file so downstream steps like tests and packagers see
 *   fully-durable bytes rather than page-cache state. esbuild +
 *   child-process builders resolve their write Promises before the file
 *   system view is durable on darwin CI runners. Skipped on Linux + Windows:
 *   Linux's `fs.writeFile` already provides the needed durability for our
 *   use, and Windows cannot `open(dir, 'r')` for the directory-flush step
 *   under its file-handle semantics.
