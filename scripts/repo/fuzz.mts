...
export function parseShmRows(
  ...
  for (let i = 0, { length } = lines; i < length; i += 1) {
    ...
    // Data rows start with the type marker `m` for shared memory.
 ...
 * after which every run dies with
 * `shmem allocation failed … check OS limits (shmall, shmmax)`. A segment is
 * an orphan only when it is OURS, has zero attachments, and its creator PID is
 * dead — live fuzz runs are untouched.
 */
...
function main(): number {
  ...
  // oxlint-disable-next-line socket/prefer-async-spawn -- sync-required
...
const SCRIPT_META: ScriptMeta = {
  describe:
    'runs the vitiate coverage-guided fuzz lane (pnpm run test:fuzz), forwarding argv to vitest',
  help: `Usage: node scripts/repo/fuzz.mts [vitest args…]
