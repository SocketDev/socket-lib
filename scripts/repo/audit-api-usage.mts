 ...
 *     and UNUSED with no reference at all. Also flags pass-through re-exports
 *     (a consumer forwarding a lib subpath through its own surface — a
 *     cleanup candidate).
...
// produced it and where it came from: which consumer repo, and whether it is a
// cascade-source path (the wheelhouse `template/` tree, which SHIPS to every
// member — so a subpath used only there is live fleet-wide, not "directly
// adopted"). The shape lets the report separate clean imports from blind spots.
...
// AST-walk one file, returning every lib reference it makes, of any shape.
export function collectRefs(
  ...
  walkSimple(source, {
    ImportDeclaration(node) {
      ...
      // with no runtime surface and no tree-shaking impact, so it's NOT a
      // blind spot — treat it as a plain named ref. The acorn-wasm parser
      // doesn't expose `importKind`, so detect `import type` from the
...
function main(): number {
  ...
  const allRefs = collectConsumerReferences(consumers)
  ...
  //   adopted     — a MEMBER repo imports it directly, which is real
  //                 downstream demand
  ...
  // used and counted as adopted, but the forwarding adds nothing — downstream
  ...
  // wheelhouse template/ is excluded because its re-exports are intentional
  // shims.
