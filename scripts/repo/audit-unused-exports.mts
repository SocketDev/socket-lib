 ...
 *   any fleet consumer repo. Complements the subpath granularity of
 *   audit-api-usage.mts with per-name granularity.
 *   Matching is identifier-based, a word-boundary token scan, so it sees
 *   through namespace imports (`ns.foo()`), re-export chains, and `require`
 *   shapes that a specifier-based audit can't attribute per name. The
 *   trade-off is conservatism: a name shared with ANY other identifier in the
 *   scanned tree counts as used, and a name defined in more than one module is
 *   reported as a collision and never flagged. False "used" is possible; false
 *   "unused" is not, except for string-built dynamic access, which nothing
 *   static can see.
