 ...
 *   `--ignore-path` resolver, and the oxfmt argv builder live here once.
 *
 *   The hazard this prevents: a bare `oxfmt --write .` (no `--ignore-path`)
 ...
 *   always threads `--ignore-path`) is what keeps `pnpm run format` and the lint
 *   gate in lock-step.
 */
...
export const FLEET_IGNORE_PATH = path.join('.config', 'fleet', '.prettierignore')
