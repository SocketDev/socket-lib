 ...
 *   platform) so a hash-locked uv install is available BEFORE the security-tools
 *   step that needs it (SkillSpector installs via a uv project + uv.lock). uv
 *   release tags have NO `v` prefix (e.g. `0.11.21`), unlike sfw/codedb/janus —
 *   so the download URL uses the bare version. Skipped (no error) when uv isn't
