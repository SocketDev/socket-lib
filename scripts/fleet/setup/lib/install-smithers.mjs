 ...
 *   AI agent-workflow orchestrator. npm-registry tarball (pure JS run via node),
 *   the same shape as npm itself: a SINGLE top-level integrity. Downloaded +
 *   SRI-verified + extracted by lib/install-tool.mjs into rack/smithers/<v>;
 *   a bin/smithers shim runs the package's bin entry (src/bin/smithers.js)
 *   through system node. Skipped (no error) when smithers is absent from
 *   external-tools.json. Imports only bootstrap-common.mjs + `node:`.
 */
