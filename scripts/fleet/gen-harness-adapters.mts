 ...
 *   rules from a host-named path; this emits a thin adapter at each.
 *
 *   Adapters are MEMBER-GENERATED (run per-repo, per-platform) and gitignored,
 *   NOT tracked. Why member-side: a tracked symlink is checked out as a plain
 *   text file on Windows (without `core.symlinks` + privilege), so it would hold
 *   the literal target path instead of the rules. Generating on the member's own
 *   OS yields a real symlink on mac/linux and a regular pointer file on Windows.
 *
 ...
 *   (no content copied — DRY).
 *
 *   Usage: node scripts/fleet/gen-harness-adapters.mts [--check]
 *     (no flag) create/repair the adapters in place.
 *     --check    report drift (missing / wrong target / stale) without writing;
 *                exit 1 if any. Used by the check-only twin.
 */
...
export function findDrift(repoRoot: string): string[] {
  ...
  for (let i = 0, { length } = ADAPTERS; i < length; i += 1) {
    ...
    if (stats.isSymbolicLink()) {
      ...
      if (got !== want) {
        drift.push(`wrong symlink target: ${adapter.dest} -> ${got} (want ${want})`)
      }
