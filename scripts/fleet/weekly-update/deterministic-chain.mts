...
export function parseLockstepReport(stdout: string): Report[] | undefined {
  ...
  try {
    const parsed = JSON.parse(stdout.slice(start)) as { reports?: Report[] | undefined }
...
// Step 1: lockstep version-pin auto-bumps. Returns the per-step result; the
// advisory rows are reported in the note so the caller can narrow the AI prompt.
export async function bumpLockstep(): Promise<ChainStepResult> {
  ...
  for (let i = 0, { length } = auto; i < length; i += 1) {
    ...
    try {
      ...
      logger.warn(`[weekly-update] lockstep bump ${row.id} failed: ${errorMessage(e)}`)
    }
  }
...
// Step 2 marker: a repo with submodules but no lockstep manifest has only
// repo-specific submodule bumps, which have no single deterministic entrypoint —
// they stay with the AI advisory pass. Recorded so the remainder is explicit.
...
// Step 3: npm deps via the existing update.mts (taze 2-pass + lockfile).
...
// Step 4: package-manager pins from external-tools.json.
...
// Step 5: registry workflow SHA pins (--fix rewrites in place).
export async function syncRegistryWorkflowPins(): Promise<ChainStepResult> {
  ...
  return {
    ...
    note: ok ? undefined : 'sync-registry-workflow-pins.mts --fix exited non-zero',
    ok,
  }
}
