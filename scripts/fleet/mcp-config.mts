...
/**
 * Render the project `.codex/hooks.json` — a PreToolUse guard that blocks edits
 * to fleet-canonical files, the Codex analog of the Claude no-fleet-fork-guard.
 * Static (server-independent): Codex discovers `<repo>/.codex/hooks.json` as
 * the project hook layer, matches `tool_name` against the regex, and runs the
 * shared cross-CLI shim which shares its block decision with every other CLI.
 */
export function renderCodexHooksConfig(): string {
  return `${JSON.stringify(
    {
      description:
        'Fleet guard: block edits to fleet-canonical files. Generated by scripts/fleet/mcp-config.mts.',
      hooks: {
        PreToolUse: [
          {
            hooks: [
              {
                command: 'node scripts/fleet/cross-cli/pretooluse-hook.mts',
                timeout: 10,
                type: 'command',
              },
            ],
            matcher: 'apply_patch|shell|local_shell',
          },
        ],
      },
    },
