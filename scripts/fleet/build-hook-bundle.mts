...
export const BUNDLE_PATH = path.join(DISPATCH_DIR, 'bundle.cjs')
...
function main(): void {
  ...
  if (!existsSync(BUNDLE_PATH)) {
    logger.error(`rolldown finished but ${BUNDLE_PATH} is missing.`)
    process.exitCode = 1
    return
  }
  logger.log(`Built ${path.relative(REPO_ROOT, BUNDLE_PATH)}.`)
}
