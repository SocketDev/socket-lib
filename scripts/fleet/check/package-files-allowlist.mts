...
export const FORBIDDEN_PUBLISHED_PATTERNS: readonly RegExp[] = [
  ...
  /(^|\/)test\//, // socket-hook: allow regex-alternation-order — `^` (start anchor) before `\/` (literal slash) reads as "either start-of-path or a slash boundary".
  /(^|\/)tests\//, // socket-hook: allow regex-alternation-order — `^` (start anchor) before `\/` (literal slash) reads as "either start-of-path or a slash boundary".
  /\.test\.[cm]?[jt]sx?$/,
  /\.spec\.[cm]?[jt]sx?$/,
  // Build/dev scripts that aren't part of the published API.
  /(^|\/)scripts\//, // socket-hook: allow regex-alternation-order — `^` (start anchor) before `\/` (literal slash) reads as "either start-of-path or a slash boundary".
  // Per-developer config dirs.
  /(^|\/)\.config\//, // socket-hook: allow regex-alternation-order
  /(^|\/)\.github\//, // socket-hook: allow regex-alternation-order
  /(^|\/)\.claude\//, // socket-hook: allow regex-alternation-order
  /(^|\/)\.git-hooks\//, // socket-hook: allow regex-alternation-order
  /(^|\/)\.vscode\//, // socket-hook: allow regex-alternation-order
  // Lockfiles + workspace metadata.
  /(^|\/)pnpm-lock\.yaml$/, // socket-hook: allow regex-alternation-order
  /(^|\/)pnpm-workspace\.yaml$/, // socket-hook: allow regex-alternation-order
]
...
export function runCheck(repoRoot: string): number {
  ...
  if (findings.length === 0) {
    logger.log('[check-package-files-allowlist] all publishable packages OK')
    return 0
  }
  logger.fail(`[check-package-files-allowlist] ${findings.length} finding(s):`)
