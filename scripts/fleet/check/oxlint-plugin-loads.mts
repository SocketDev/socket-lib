 ...
 *   helper, a renamed export), every `socket/` rule is disabled. oxlint surfaces
 *   this only as a `Failed to load JS plugin` warning on stderr — whether that
 *   warning gates the run depends on oxlint's exit behavior, which has varied
 *   by version + invocation mode (the originating incident saw a green lint
 *   with the plugin silently not loaded). Relying on that incidental exit code
 *   is fragile; this gate asserts load EXPLICITLY and fails closed with a clear
 *   "every socket/ rule is disabled" message. The static surfaces don't help:
 *   `sync-oxlint-rules` and the `oxlint-rule-activations` check only verify a
 *   rule is imported in `index.mts` and activated in `oxlintrc.json` — a
 *   statically-present import that throws at runtime passes both.
 *
 *   Checks (the second is something oxlint NEVER does):
 ...
 *      expected count is derived from the file listing.
 *
 *   Pairs with the edit-time `.claude/hooks/fleet/oxlint-plugin-load-guard/`
 *   (defense in depth). Exit codes: 0 — plugin loads + count matches; 1 — load
 *   threw, empty rules, or count mismatch.
 *
 *   **Why:** memory `project_oxlint_plugin_load_silent_fail` — a bad import in
 *   any rule disables ALL socket rules; green lint ≠ plugin loaded. Promoted to
 *   a gate after verifying plugin load by hand one too many times (2026-06-03).
 */
