 ...
 *   `.github/workflows/` and runs anything on a `schedule:` — so the fallback is
 *   shipped as `weekly-update-non-gh-aw.yml.disabled` (the `.disabled` extension
 *   makes it invisible to the Actions loader). `weekly-update-workflow.mts
 *   enable` copies it to the live `.yml` for a one-off run, then `disable`
 *   re-hides it; the enabled `.yml` is meant to be transient + untracked.
 *
 *   If the ENABLED `.yml` is ever committed, it auto-runs weekly in every repo
 *   the file cascades to — an accidental fleet-wide scheduled workflow with
 *   nobody intending it. This check fails when the enabled form is git-tracked,
 *   so the accident can't land. The `.yml.disabled` form is expected + ignored.
 *
 *   Usage: node scripts/fleet/check/weekly-update-fallback-is-disabled.mts [--quiet]
 */
