...
export function listActionNames(repoRoot: string): string[] {
  ...
  return readdirSync(dir, { withFileTypes: true })
    .filter(
      d =>
        d.isDirectory() &&
        existsSync(path.join(dir, d.name, 'action.yml')),
    )
    .map(d => d.name)
    .sort()
}
...
export function closureFor(
  ...
  for (let i = 0, { length } = unit.deps, deps = unit.deps; i < length; i += 1) {
    const dep = deps[i]!
  ...
  for (const unit of units.values()) {
    for (let i = 0, { length } = unit.deps, deps = unit.deps; i < length; i += 1) {
      ...
      if (!git.isReachable(pin.sha, base)) {
        findings.push({ file, dep: depId, sha: pin.sha, verdict: 'unreachable' })
        continue
      }
...
export function resolveBase(): string {
  ...
  for (const name of ['main', 'master']) {
    if (git(['show-ref', '--verify', '--quiet', `refs/remotes/origin/${name}`]).status === 0) {
      return `origin/${name}`
    }
  }
  return 'origin/main'
