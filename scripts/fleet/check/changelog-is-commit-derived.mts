 ...
 *   from the Conventional Commits being released (run `node scripts/fleet/bump.mts`),
 *   never hand-written ahead of the tag.
 *
 *   Fires only for a PENDING release — when `package.json` version is ahead of
 *   the last `v<semver>` tag. For that pending version it regenerates the entry
 *   from the commits since the last tag and fails when the committed entry's
 *   bullets don't match. A published version (version == last tag) is historical
 *   and not re-validated. Fail-open: any uncertainty (no tag, shallow clone,
 *   unreadable CHANGELOG) skips rather than false-fails.
 *
 *   Usage: node scripts/fleet/check/changelog-is-commit-derived.mts
 */
...
async function main(): Promise<void> {
  ...
  if (missing.length) {
    detail.push(`  ${missing.length} released commit(s) missing from the entry:`)
