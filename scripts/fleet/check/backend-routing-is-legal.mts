  ...
  for (const match of text.matchAll(PREFERENCE_ORDER_RE)) {
    ...
    for (const q of body.matchAll(QUOTED_RE)) {
      ...
      if (!KNOWN_BACKENDS.has(name)) {
        out.push({
          detail: `preferenceOrder names unknown backend "${name}" — not in @socketsecurity/lib/ai/backends BACKENDS (${[...KNOWN_BACKENDS].toSorted().join(', ')}). Fix the name or add the backend to the registry.`,
          file,
          line,
        })
