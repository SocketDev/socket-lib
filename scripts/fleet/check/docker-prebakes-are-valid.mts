...
interface PrebakeManifest {
  readonly prebakes?: unknown
  readonly registry?: unknown
}

function collectProblems(manifest: PrebakeManifest): readonly string[] {
  ...
  const byName = new Map(
    prebakes.map(raw => [(raw as PrebakeEntry).name as string, raw as PrebakeEntry]),
  )
...
function main(): void {
  ...
  try {
    ...
    logger.fail(
      `docker-prebakes.json is not valid JSON: ${e instanceof Error ? e.message : String(e)}`,
    )
    process.exitCode = 1
    return
  }
