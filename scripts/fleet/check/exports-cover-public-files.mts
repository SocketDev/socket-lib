 ...
 *
 *   Usage: node scripts/fleet/check/exports-cover-public-files.mts [--quiet]
 */
...
export async function runCheck(repoRoot: string): Promise<number> {
  ...
  if (!findings.length) {
    logger.log(
      '[check-exports-cover-public-files] every exports target resolves and every public file is exported.',
    )
    return 0
  }
  logger.fail(
    `[check-exports-cover-public-files] ${findings.length} finding(s):`,
  )
