 ...
 *   scripts/fleet/check/provenance.mts <name>
 *
 *   # last 10 versions
 *
 *   node scripts/fleet/check/provenance.mts <name> --all.
 *
 *   # every published version
 *
 *   node scripts/fleet/check/provenance.mts <name> --version 1.2.3.
 *
 *   # one specific version
 *
 *   node scripts/fleet/check/provenance.mts <name> --json.
 *
 *   # pipe to jq / scripts
 *
...
async function main(): Promise<void> {
  ...
  if (values['help'] || positionals.length === 0) {
    logger.log(
      'Usage: node scripts/fleet/check/provenance.mts <name> [options]',
    )
    logger.log('')
  ...
  const allVersions = Object.keys(versions).toSorted(compareSemverDesc)
