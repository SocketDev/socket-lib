...
//
// Three shapes are scanned across the source tree (scripts + skills + hooks):
//   1. spawnAiAgent({ ... }) / Workflow agent({ ... }) calls — the argument
//      object must name BOTH `model` and `effort`. (A spread profile like
//      AI_PROFILE.edit never carries either, so the spread doesn't satisfy the
//      pairing — the call must name both keys.) When the call names a literal
//      model above the cheapest, or a literal effort above `low`, an adjacent
//      justifying comment is required.
...
export function scanSpawnCalls(
  ...
  // per-agent spawn. Both take the same uniform options object.
