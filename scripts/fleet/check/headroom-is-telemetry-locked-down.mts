 ...
 *   .claude/reports/headroom-telemetry-audit.md). The installed `bin/headroom`
 *   is a wrapper that exports HEADROOM_LOCKDOWN_ENV before exec, disabling both
 *   for every invocation.
 *
 *   This check IMPORTS the typed lockdown exports (never source-sniffs — per
 *   socket/no-source-sniffing) and asserts (1) the lockdown disables telemetry
 *   AND the model fetch, and (2) the generated wrapper actually exports them
 *   before exec. The lib also throws at import if the lockdown is weakened
 *   (fail-closed), so a regression fails here AND at every `bin/headroom` build.
 *
 *   Usage: node scripts/fleet/check/headroom-is-telemetry-locked-down.mts [--quiet]
 */
...
// scripts/fleet/check/ → repo root → the headroom installer lib. Same relative
// shape in the wheelhouse (template/base/) and in a cascaded member.
...
async function main(): Promise<number> {
  if (!existsSync(HEADROOM_LIB)) {
    logger.log('headroom-is-telemetry-locked-down: no headroom installer (n/a).')
    return 0
  }
