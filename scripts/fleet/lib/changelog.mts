 ...
 *   `changelog-is-generated` guard blocks hand-edits so the derivation stays the
 *   single source of truth.
 *
 *   Conventional Commits 1.0: `<type>[(scope)][!]: <description>`. A `!` before
 *   the colon, or a `BREAKING CHANGE:` line in the body, marks a breaking change.
 *   Only user-visible types reach the CHANGELOG (feat / fix / perf / revert);
 *   chore / ci / docs / test / style / build / refactor are omitted because the
 *   CHANGELOG records user-visible behavior, not internal churn.
 */
 ...
 * nothing user-visible landed — the caller decides whether to refuse the release
 * or force a patch.
 */
export function bumpLevelFor(commits: readonly ConventionalCommit[]): BumpLevel | undefined {
 ...
 * `git@github.com:Org/Repo.git`, …) to a plain `https://github.com/Org/Repo`
 * base, or `undefined` when it can't. Used to link the version heading to its
 * release.
 */
export function repoBaseUrl(repositoryUrl: string | undefined): string | undefined {
  if (!repositoryUrl) {
    return undefined
  }
  const m =
    /github\.com[/:](?<owner>[^/]+)\/(?<repo>[^/.]+)/.exec(repositoryUrl)
  if (!m?.groups) {
    return undefined
  }
