...
export function parseTripletSegment(name: string): PackAppTriplet | undefined {
  ...
  // shortenings.
  // `PACK_APP_TRIPLETS` is a shared module-level const, so the spread copies it
  // first; an in-place sort would mutate the constant list every caller shares.
  // .toSorted() would trip socket/no-runtime-features-below-engine-floor in
  // cascaded Node-18 repos.
  // oxlint-disable-next-line unicorn/no-array-sort -- `PACK_APP_TRIPLETS`
