 ...
 */
async function findDistFiles(distPath, isRoot = true) {
  const files = []
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        // Skip dist/external directory (contains bundled external deps with their own requires)
        if (isRoot && entry.name === 'external') {
          continue
        }
        files.push(...(await findDistFiles(fullPath, false)))
      } else if (
