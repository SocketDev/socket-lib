...
export const RULE_GUIDANCE: Readonly<Record<string, string>> = {
  ...
  'socket/prefer-async-spawn':
    "Replace `node:child_process`'s `spawnSync` with one of two `@socketsecurity/lib/spawn` exports — preferred first: (a) async `spawn` (returns a thenable yielding `{ code, stdout, stderr }`; await it), or (b) `spawnSync` from the same lib module when the caller is genuinely sync. Both replacements drop the `node:child_process` import. Decision per callsite: if the enclosing function is `async` or can be made async without breaking its sync callers, use (a); otherwise use (b). Process: (1) inspect every `spawnSync(` callsite in the file, (2) pick (a) or (b) per callsite, (3) update the import to `import { spawn } from '@socketsecurity/lib/spawn/spawn'` (or `import { spawnSync } from '@socketsecurity/lib/spawn/spawn'`) — the lib's `spawnSync` is a drop-in replacement signature-wise (same args, same return shape) so callsite bodies don't change when using (b). CRITICAL: never end the rewrite with the `node:child_process` import swapped but a callsite still referencing the old `spawnSync` — that produces an undefined-symbol crash at runtime. If you use (a) for some callsites and (b) for others, import both: `import { spawn, spawnSync } from '@socketsecurity/lib/spawn/spawn'`.",
  'socket/prefer-undefined-over-null':
