...
2. **`bumpAndCommit` stage** — in interactive mode, run `pnpm test` FIRST (and, for a `locked` row, confirm the human approval is in hand). Then land the bump deterministically with `node scripts/fleet/lockstep/auto-bump.mts --apply --id <row-id> --target-tag <tag>` — it checks out the tag in the submodule, rewrites `lockstep.json`'s `pinned_tag`/`pinned_sha`, regenerates the `.gitmodules` `# <name>-<version> sha256:…` annotation via `gen-gitmodules-hash.mts --set`, and commits `chore(deps): bump <upstream> to <tag>`. Validate after with `pnpm run lockstep` (exits 0 or 2). The script prints `{ committed, state: bumped|skipped-no-row|skipped-no-submodule, pinnedSha, targetTag }` — roll that into `RESULT_SCHEMA`. A test failure means you DON'T call `--apply` for that row; record it as `test-failed` and drop the item to `null` (filter before the Phase-5 report).
