...
`commit-message-format-guard`. The collapse + the push are both history-rewriting operations that
`no-revert-guard` gates; the runner sets an inline **`SQUASH_HISTORY=1`** sentinel limited to exactly
