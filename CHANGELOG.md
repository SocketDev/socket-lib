...
- **`dlx/package` `installPath` option** — new field on `EnsurePackageInstallOptions` (and `DlxPackageOptions` via inheritance) that bypasses the default `~/.socket/_dlx/<cacheKey>/` cache layout. When set, the package lands directly at `<installPath>/node_modules/<packageName>/` — no cacheKey subdirectory. Purpose: build pipelines that want the install gitignored alongside their own outputs and walkable by tools resolving through `node_modules` (the original use case is `socket-btm`'s `ink-builder` bundling ink + its runtime deps via esbuild). The caller owns per-spec separation: calling twice with the same `installPath` but different specs overwrites the earlier install — pass `force: true` to accept that, or use a different `installPath` per spec.
