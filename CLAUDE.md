...
Use fleet MCP `rule_doc` with a topic name to retrieve a bounded rule section. Read only rules relevant to the operation.

- Identify users by git credentials; use "you/your" directly; shorthand phrases have fixed meanings. Named on-demand sync: "cascade `<target>`" = one slice, "dogfood `<target>`" = self-sync, "cascade `<target>` to `<repo>`" = one member. [`vocabulary`](docs/fleet/agents.md/vocabulary.md)
...
- Primary checkout stays on the default branch; branch work goes in a `git worktree`. Companion sessions are quick checks, blocked past a 1-min budget. Land often. [`parallel-claude-sessions`](docs/fleet/agents.md/parallel-claude-sessions.md)
...
- Conventional Commits `<type>(<scope>): <description>`, lowercase, NO AI attribution, in commits, every GitHub prose surface, and external MCP surfaces (Linear, Slack). PRs stay small, one logical feature/fix around 200 changed lines. Never open a PR from the default branch. [`commit-cadence-format`](docs/fleet/agents.md/commit-cadence-format.md)
...
- zsh does not word-split `$var`: a space-joined list in a variable passes as ONE arg. Dedup the install tree: no avoidable cross-major duplicate, and every `@socketregistry/*` hardened drop-in is redirected via `overrides:`. [`tooling`](docs/fleet/agents.md/tooling.md)
...
- Fix a lint/type/test error or broken comment in your reading window in a sibling commit. Default to perfectionist. [`judgment-and-self-evaluation`](docs/fleet/agents.md/judgment-and-self-evaluation.md)
...
- Finish a change, then commit it; never end a turn with a dirty worktree. Smallest chunks, land ASAP; never checkout/switch mid-queue. Before removing a branch as redundant, verify the kept branch contains its content. (`.claude/hooks/fleet/branch-worktree-sweep-nudge/`). [`worktree-hygiene`](docs/fleet/agents.md/worktree-hygiene.md)
...
- Fleet hooks bundle into `.claude/hooks/fleet/_dist/fleet-pack.cjs`; rebuild after touching a source. A snapshotted hook NEVER uses dynamic `import()`: use `process.getBuiltinModule('node:x')`, or mark it `@dispatch-snapshot-exclude`. [`hook-bundle`](docs/fleet/agents.md/hook-bundle.md)
...
- When the same finding fires twice, promote it to a rule in CLAUDE.md, a hook, or a skill. Every memory entry's frontmatter needs an `enforcement:` disposition. [`memory-codification`](docs/fleet/agents.md/memory-codification.md)
- For non-trivial work, write the plan as a deliverable: numbered steps, named files and rules, second opinion for fleet-shared changes. Plans go to `<repo-root>/.claude/plans/<name>.md`, reports to `<repo-root>/.claude/reports/<name>.md`. [`plan-storage`](docs/fleet/agents.md/plan-storage.md)
- Markdown filenames are `lowercase-with-hyphens.md` under `docs/` or `.claude/`. An exported name carries a domain word; a bare single generic token (`create`/`parse`/`get`) is grep noise. Fixture names in tests are fake but DESCRIPTIVE (`example.js`, `@example/module`), never single letters. (`scripts/fleet/check/fixture-names-are-descriptive.mts`). A test fixture names a FICTIONAL person (`octocat`, `example-user`), never a real maintainer. (`scripts/fleet/check/test-identities-are-fictional.mts`). [`code-style`](docs/fleet/agents.md/code-style.md)
...
- New lint rules default `"error"` with `fixable: 'code'`; oxlint + oxfmt only, no ESLint/Prettier/Biome. `lint`/`fix` default to the MODIFIED scope, so a clean tree checks NOTHING. [`lint-rules`](docs/fleet/agents.md/lint-rules.md)
...
- A feature needs a code-as-law check, unit/integration/e2e tests, preflight wiring, and 90%+ coverage. When idle, increasing coverage toward 90%+ is the default pickup. [`feature-completeness`](docs/fleet/agents.md/feature-completeness.md)
...
- Repo-root `upstream/<name>` is the ONLY submodule home, never `packages/*/upstream/*` or `test/fixtures/*`. (`scripts/fleet/check/submodules-are-rooted-in-upstream.mts`) Never git-track an `upstream/` gitlink. [`upstream-references`](docs/fleet/agents.md/upstream-references.md)
...
- Fleet/repo segmentation on every surface; a `-guard` BLOCKS, a `-nudge` NUDGES. A written mermaid fence gets rewritten GitHub-safe at edit time. [`hook-registry`](docs/fleet/agents.md/hook-registry.md)
...
- Stale GitHub Actions run history is pruned weekly by `scripts/fleet/prune-workflow-runs.mts`. Actions cache over 10 GB silently LRU-evicts itself. [`workflow-run-retention`](docs/fleet/agents.md/workflow-run-retention.md)
...
- 🚨 NEVER `process.chdir()`: pass `{ cwd }` and absolute paths; NEVER `--` before vitest test paths - runs ALL tests. [`architecture`](docs/agents.md/repo/architecture.md)
