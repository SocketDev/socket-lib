...
🚨 **Supply-chain hygiene.** New deps Socket-scored at edit time; 7-day `minimumReleaseAge` soak is malware protection (bypass `Allow minimumReleaseAge bypass`); soak-bypass entries need `# published: YYYY-MM-DD | removable: YYYY-MM-DD` annotations. Dep overrides in `pnpm-workspace.yaml`, never `package.json` `pnpm.overrides` (bypass `Allow package-json-overrides bypass`). **Never weaken a trust gate** (`trustPolicy: no-downgrade`, `--config.trustPolicy=trust-all`, `blockExoticSubdeps`) — fix stale lockfiles via the soak/exclude entry; the bypass `Allow trust-downgrade bypass` is single-use and not persisted (enforced by `.claude/hooks/fleet/check-new-deps/`, enforced by `.claude/hooks/fleet/minimum-release-age-guard/`, enforced by `.claude/hooks/fleet/soak-exclude-date-annotation-guard/`, enforced by `.claude/hooks/fleet/no-package-json-pnpm-overrides-guard/`, enforced by `.claude/hooks/fleet/trust-downgrade-guard/`).
...
Citation gate for `new-hook-claude-md-guard`; each hook's README docs trigger + bypass. Hooks live under `.claude/hooks/fleet/<name>/` (mirrors `docs/claude.md/{fleet,repo}/` + `scripts/{fleet,repo}/`). Host-repo-only hooks live under `.claude/hooks/repo/<name>/` and are exempt from the citation gate.

enforced by `.claude/hooks/fleet/{codex-no-write-guard,concurrent-cargo-build-guard,broken-hook-detector,answer-passing-questions-reminder,answer-status-requests-reminder,avoid-cd-reminder,actionlint-on-workflow-edit,commit-author-guard,enterprise-push-property-reminder,no-empty-commit-guard,no-external-issue-ref-guard,no-orphaned-staging,pr-vs-push-default-reminder,prefer-rebase-over-revert-guard,provenance-publish-reminder,pull-request-target-guard,scan-label-in-commit-guard,version-bump-order-guard,workflow-uses-comment-guard,workflow-yaml-multiline-body-guard,extension-build-current-guard,file-size-reminder,inline-script-defer-guard,node-modules-staging-guard,no-structured-clone-prefer-json-guard,parallel-agent-edit-guard,path-guard,paths-mts-inherit-guard,plugin-patch-format-guard,pointer-comment-guard,private-name-guard,public-surface-reminder,uses-sha-verify-guard,vitest-include-vs-node-test-guard,auth-rotation-reminder,no-blind-keychain-read-guard,no-token-in-dotenv-guard,token-guard,comment-tone-reminder,judgment-reminder,socket-token-minifier-start,stale-process-sweeper,sweep-ds-store}/`.
