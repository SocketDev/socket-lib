/**
 * @file Claude Code PreToolUse hook — clone-reviewed-repo-nudge.
 *
...
import { notify, bashGuard, defineHook, runHook } from '../_shared/guard.mts'
...
function buildMissingFlagsMessage(
  owner: string,
  repo: string,
...
function buildCloneForReviewMessage(owner: string, repo: string): string {
  const dest = `~/.socket/_wheelhouse/repo-clones/${repoClonesName(owner, repo)}/`
  return [
...
export const check = bashGuard(command => {
  ...
  if (findInvocation(command, { binary: 'gh' })) {
    for (const cmd of commandsFor(command, 'gh')) {
      ...
      if (repo) {
        return notify(buildCloneForReviewMessage(repo.owner, repo.repo))
      }
    }
  }
  ...
  for (const cmd of commandsFor(command, 'git')) {
    ...
    if (result && result.missing.length) {
      return notify(buildMissingFlagsMessage(result.owner, result.repo, result.missing))
    }
  }
  return undefined
})
