...
export function parseAuthorOverride(command: string): GitAuthor | undefined {
  ...
  const authorEq = /--author=(?<q>['"]?)(?<name>[^'"<>]+)\s*<(?<email>[^>]+)>\k<q>/i.exec(command)
  if (authorEq) {
    return { name: authorEq.groups!.name!.trim(), email: authorEq.groups!.email!.trim() }
  }
  // --author "Name <email>"
  const authorSpace = /--author\s+(?<q>['"])(?<name>[^'"<>]+)\s*<(?<email>[^>]+)>\k<q>/i.exec(command)
  if (authorSpace) {
    return { name: authorSpace.groups!.name!.trim(), email: authorSpace.groups!.email!.trim() }
  }
  // -c user.email=...
  const cEmail = /-c\s+user\.email=(?<email>[^\s'"]+)/i.exec(command)
  const cName = /-c\s+user\.name=(?:(?<q>['"])(?<quotedName>[^'"]+)\k<q>|(?<bareName>[^\s]+))/i.exec(command)
  if (cEmail || cName) {
    return {
      email: cEmail?.groups?.email,
      name: cName ? (cName.groups?.quotedName ?? cName.groups?.bareName) : undefined,
    }
  }
  return undefined
}
