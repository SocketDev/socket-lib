...
export const check = bashGuard((command, payload) => {
  ...
  if (
    (payload as { hook_event_name?: unknown }).hook_event_name !== 'PostToolUse'
  ) {
    return undefined
  }
  ...
  const toolResponse = (payload as { tool_response?: unknown }).tool_response
  ...
  const currentValue = slug
    ...
    : undefined
  return notify(formatReminder(slug, currentValue))
})
