...
//
// Fail-open on any IO error: a guard bug must never block a legitimate
// session.
...
import { hookRepoRoot, resolveRepoRoot } from '../_shared/repo-root.mts'
import { verdictLine } from '../_shared/verdict.mts'
import { reapStaleCompanions } from './reap.mts'
...
// How often the sweep may run. It is throttled on wall clock rather than tied
// to a companion's own tool calls, because the processes it catches belong to
// sessions that STOPPED making tool calls — waiting for a runaway's own session
// to check in is waiting for the thing that already failed to happen.
export const REAP_INTERVAL_MS = 5 * 60_000

// The stamp recording the last sweep, beside the per-companion markers.
export function reapStampFile(projectDir: string): string {
  return path.join(
    resolveRepoRoot(projectDir),
    '.cache',
    'fleet',
...
// Whether enough time has passed since the last sweep. An unstamped store
// sweeps immediately, which is what makes the first call on a fresh machine —
// or after the store is cleared — do something.
...
// Housekeeping riding along on someone else's tool call, so every failure is
// swallowed: this must never be the reason a tool call fails.
function sweepStaleCompanions(projectDir: string, nowMs: number): void {
  try {
    const stamp = reapStampFile(projectDir)
    if (!shouldReap(readStartMs(stamp), nowMs, REAP_INTERVAL_MS)) {
      return
    }
    stampStartMs(stamp, nowMs)
    reapStaleCompanions()
  } catch {}
}
...
export function check(
  ...
  // The checkout to store the companion marker in. Defaults to this hook's
  // own, and is a parameter so a test can point it at a scratch checkout.
  projectDir: string = hookRepoRoot(payload.cwd ?? import.meta.url),
): GuardResult {
  const companionId = codexCompanionId(process.env)
  if (!companionId) {
    return undefined
  }
  // Ahead of the self-id early return, so a PRIMARY session sweeps too. Only a
  // primary session is reliably still making tool calls, which makes it the one
  // that can clean up after companions that are not.
  sweepStaleCompanions(projectDir, Date.now())
  ...
  return block(
    [
      verdictLine(
        ...
        `companion ${companionId.slice(0, 8)}… has run ${minutes} min, budget is ${budgetLabel} - long companions monopolize the shared checkout.`,
      ),
