...
test('FLAGS deferring a lint issue as a follow-up', () => {
  const transcriptPath = makeTranscript([
    { role: 'user', text: 'fix all the lint issues' },
    {
      role: 'assistant',
      text: 'Fixed the main ones. The ~28 string-glyph sites is a follow-up.',
    },
  ])
  const { stderr, exitCode } = runHook(transcriptPath)
  assert.equal(exitCode, 0)
...
test('FLAGS a "remaining backlog" wrap-up', () => {
  const transcriptPath = makeTranscript([
    { role: 'user', text: 'do them all, keep going' },
    {
      role: 'assistant',
      text: 'Landed the first batch. Remaining backlog: #30, #33, #34.',
    },
  ])
  const { stderr, exitCode } = runHook(transcriptPath)
  assert.equal(exitCode, 0)
  assert.match(stderr, /remaining\s+(backlog|to-?dos?|items|tasks|work)/i)
})

test('FLAGS a budget excuse for an unfinished queue', () => {
  const transcriptPath = makeTranscript([
    { role: 'user', text: 'finish all the todos' },
    {
      role: 'assistant',
      text: "Did three. The rest I didn't have budget for this turn.",
    },
  ])
  const { stderr, exitCode } = runHook(transcriptPath)
  assert.equal(exitCode, 0)
  assert.match(stderr, /didn'?t\s+have.*budget/i)
})

test('does NOT flag when the user authorized stopping', () => {
  const transcriptPath = makeTranscript([
    { role: 'user', text: 'ok stop here, we are done for today' },
    {
      role: 'assistant',
      text: 'Sounds good. The remaining backlog is #30 and #33 for next time.',
    },
  ])
  const { stderr } = runHook(transcriptPath)
  assert.equal(stderr, '')
})
