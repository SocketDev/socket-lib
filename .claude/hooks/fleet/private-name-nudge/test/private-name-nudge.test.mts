...
test('escalates the reminder when committing OUTSIDE a fleet repo', async () => {
  // A real git repo with no remote + no fleet marker → non-fleet. The nudge
  // must add the external-repo escalation (mere mention of internal Socket
  // infrastructure discloses it). Still exit 0 — attention priming, not a block.
  const dir = mkdtempSync(path.join(os.tmpdir(), 'pnn-nonfleet-'))
  await spawn('git', ['init', dir])
  const { code, stderr } = await runHook({
    tool_name: 'Bash',
    tool_input: { command: 'git commit -m "ship feature"' },
    cwd: dir,
  })
  assert.equal(code, 0)
  assert.ok(
    stderr.includes('OUTSIDE the fleet'),
    `expected the external-repo escalation; got: ${stderr}`,
  )
})
