...
import { PROSE_PATTERNS, findProseAntipatterns } from '../patterns.mts'
...
test('the disable env var short-circuits', () => {
  const { exitCode } = runGuard(
    { file_path: '/p/socket-lib/CHANGELOG.md', content: DIRTY },
    { env: { ...process.env, SOCKET_PROSE_ANTIPATTERN_GUARD_DISABLED: '1' } },
  )
  assert.equal(exitCode, 0)
})
