 ...
 *   the legacy socket-token-minifier. headroom-ai ships on PyPI; we pin it via a
 *   committed `headroom/pyproject.toml` + `headroom/uv.lock` (the [proxy] extra,
 *   no pytorch) and `uv sync --locked` the exact closure into the content-
 *   addressed `_dlx` hash store — never the project dir — so the install is
 *   fully `~/.socket/_dlx` contained and reuses the uv-managed CPython already
 *   there. Readable handles (rack alias + bin shim) layer over the hash,
 *   mirroring install-sfw.mts. Lives in its own file because installers.mts is
 *   at the 1000-line hard cap.
 *
 *   The three-way pin (uv.lock + pyproject + external-tools.json version) is
 *   enforced by headroom-pin-is-consistent.mts.
 */
...
export function lockdownViolations(
  ...
  for (const key of Object.keys(REQUIRED_LOCKDOWN)) {
    ...
    if (applied[key] !== want) {
      out.push(`${key} must be "${want}" (telemetry/model phone-home); saw "${applied[key] ?? '(unset)'}"`)
    }
  }
  return out
}
...
export async function setupHeadroom(version: string): Promise<boolean> {
  ...
  if (!existsSync(pyproject) || !existsSync(uvLock)) {
    ...
    logger.error('  fix:   restore the project files (run `uv lock` to rebuild)')
    return false
  }
  ...
  await fs.rm(binLink, { force: true })
  await fs.writeFile(
    binLink,
    isWin
      ? lockdownWrapperScriptWin(venvBin)
      : lockdownWrapperScript(venvBin),
  )
  if (!isWin) {
    await fs.chmod(binLink, 0o755)
  }

  logger.log(`headroom-ai@${version} ready at ${binLink} (telemetry + model fetch locked off)`)
  logger.log(`  → ${venvBin}`)
  return true
}
