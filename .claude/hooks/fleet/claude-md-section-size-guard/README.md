...
Complements `claude-md-size-guard` (40KB byte cap on the whole block) by enforcing two per-section caps inside the block — both metrics of one concern, "this section is too big". Each `### Section heading` inside the `<!-- BEGIN <fleet-canonical> -->` … `<!-- END </fleet-canonical> -->` markers gets at most:
