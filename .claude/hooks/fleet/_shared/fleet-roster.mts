...
/**
 * True when the checkout at `repoRoot` is opted into the squash-history cadence.
 * For such a repo, local <default-branch> is canonical and origin holds the
 * pre-squash history — a diverged / orphan main is EXPECTED, resolved by a
 * force-push (`SQUASH_HISTORY=1 git push --force-with-lease`), never a fast-land
 * cherry-pick onto origin.
 */
