...
export function uppercaseFirstLetterAfterMarker(line: string): string {
  const m = line.match(/^(?<prefix>\s*(?:\/\/|\/\*|\*|#|-)\s*)(?<firstChar>[a-zA-Z])/)
  if (!m) {
    return line
  }
