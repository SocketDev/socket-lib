...
description: End-to-end release of a single-package fleet repo — pre-bump wave, public-facing CHANGELOG, a `chore: bump version to X.Y.Z` commit, a `vX.Y.Z` tag, a CI staged-publish (OIDC trusted publisher + provenance) via `scripts/fleet/publish.mts --staged`, then a local human 2FA `--approve` (web-OTP browser flow). Use when shipping a new version of a repo whose `publish` script is the canonical `scripts/fleet/publish.mts`. NOT for socket-registry / socket-addon, which keep a bespoke multi-package publish.mts.
user-invocable: true
...
`scripts/fleet/publish.mts` — a staged upload in CI under the OIDC trusted-publisher
...
2. **Bump — scripted, never by hand.** `node scripts/fleet/bump.mts` derives the next version
   ...
   with `node scripts/fleet/bump.mts --dry-run` first. Do NOT hand-edit `CHANGELOG.md` — a
...
3. **Push the bump.**
4. **CI stages.** Trigger the publish workflow; it runs `node scripts/fleet/publish.mts
...
5. **Approve — human, local, real terminal.** Run `node scripts/fleet/publish.mts --approve`,
