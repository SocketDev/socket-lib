...
export async function readPins(): Promise<readonly PluginPin[]> {
  ...
  for (let i = 0, { length } = manifest.plugins, { plugins } = manifest; i < length; i += 1) {
...
export async function fetchPristine(options: {
  ...
  const segments = [pin.owner, pin.repo, pin.sha, pin.path, file].filter(Boolean)
...
export async function patchDryRun(options: {
  ...
  const { cwd, patchBody, reverse } = { __proto__: null, ...options } as typeof options
  const patchFile = path.join(os.tmpdir(), `regen-body-${process.pid}-${Date.now()}.patch`)
...
export async function classifyPatch(options: {
  ...
  const reverse = await patchDryRun({ cwd: aDir, patchBody: patch.body, reverse: true })
  if (reverse === 0) {
    return 'upstreamed'
  }
  return 'stale'
...
export async function rebuildDiff(options: {
  ...
  for (let i = 0, { length } = targetPaths; i < length; i += 1) {
    ...
    try {
      ...
      const err = e as { code?: number | string | undefined; stdout?: string | undefined } | undefined
...
export function normalizeDiffPaths(options: {
  ...
  const { aPath, bPath, file, raw } = { __proto__: null, ...options } as typeof options
  const out: string[] = []
...
export function restampHeader(options: {
  ...
  const { header, pin, version } = { __proto__: null, ...options } as typeof options
...
export async function mkdtempStaging(label: string): Promise<string> {
  const dir = path.join(os.tmpdir(), `plugin-patch-${label}-${process.pid}-${Date.now()}`)
...
export function resolvePatchPath(patchArg: string): string {
  const abs = path.isAbsolute(patchArg) ? patchArg : path.join(PATCHES_DIR, patchArg)
  if (!existsSync(abs)) {
...
export function pinForPatch(options: {
  ...
  if (!pin) {
    throw new Error(`no marketplace pin for plugin ${patch.plugin || '(unknown)'}`)
  }
  return pin
}
...
export async function main(argv: readonly string[]): Promise<void> {
  ...
  if (command === 'classify') {
    ...
    entries.sort()
    const report: Record<string, PatchClassification> = { __proto__: null } as Record<
      string,
      PatchClassification
    >
    for (let i = 0, { length } = entries; i < length; i += 1) {
      const name = entries[i]!
      const patch = parsePatch(await readFile(path.join(PATCHES_DIR, name), 'utf8'))
  ...
  if (command === 'rebuild') {
    ...
    const patch = parsePatch(await readFile(resolvePatchPath(patchArg), 'utf8'))
    const pin = pinForPatch({ patch, pins })
    const body = await rebuildDiff({ dir: dirArg, targetPaths: patch.targetPaths })
    const header = restampHeader({ header: patch.header, pin })
    const full = `${header}\n${body}`
    const valid = await validatePatch({ body, patch, pin })
    if (!valid) {
      logger.fail('regenerated diff does not apply cleanly (patch --dry-run != 0)')
      process.exitCode = 1
      return
    }
    logger.log(full)
