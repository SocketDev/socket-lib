...
export async function probePricing(cwd: string): Promise<DiscoveryItem> {
  ...
  const ageDays = Math.floor(
    (Date.now() - snapshot.getTime()) / 86_400_000,
  )
  ...
  return makeItem({
    actionable: true,
    applies: true,
    items: [`model-pricing snapshot is ${String(ageDays)} days old — re-source`],
  })
}
  ...
  try {
    ...
    if (isSpawnError(e) && typeof e.code === 'number' && e.code === 1) {
      return makeItem({
        actionable: true,
        applies: true,
        items: ['registry workflow SHA pins are stale — run updating-workflows'],
      })
    }
    return makeItem({
      applies: true,
