...
export type FindingState = 'already-fixed' | 'false-positive' | 'fixed' | 'wont-fix'
...
export async function resolveOwnerRepo(): Promise<OwnerRepo> {
  ...
  return { __proto__: null, owner: parsed.owner, repo: parsed.repo } as OwnerRepo
}
...
export async function findThreadId(options: {
  ...
  const { comment, owner, pr, repo } = { __proto__: null, ...options } as typeof options
  const query = `
...
export async function resolveThread(threadId: string): Promise<void> {
  ...
  await gh(['api', 'graphql', '-f', `query=${mutation}`, '-f', `threadId=${threadId}`])
}

/**
 * Find the PR number that an inline review comment belongs to.
 */
export async function prForComment(options: OwnerRepo & { comment: number }): Promise<number> {
  const { comment, owner, repo } = { __proto__: null, ...options } as typeof options
  const out = await gh([
    'api',
...
export async function replyToFinding(options: {
  ...
  const { body, comment, state } = { __proto__: null, ...options } as typeof options
  const { owner, repo } = await resolveOwnerRepo()
 ...
 */
export async function resolveRepliedThreads(pr: number): Promise<readonly string[]> {
  const { owner, repo } = await resolveOwnerRepo()
  const query = `
    ...
    data: {
      repository: {
        pullRequest: {
          reviewThreads: {
            nodes: ReadonlyArray<{
              comments: { nodes: ReadonlyArray<{ author: { login: string } | null }> }
              id: string
              isResolved: boolean
            }>
          }
...
> {
  ...
  const results: Array<{ finding: BugbotFinding; fixCommits: readonly string[] }> = []
  for (let i = 0, { length } = findings; i < length; i += 1) {
    const finding = findings[i]!
    const fixCommits = await gitCommitsTouchingSince(finding.commitId, finding.path)
    results.push({ __proto__: null, finding, fixCommits } as (typeof results)[number])
  }
  return results
}
...
export async function gitCommitsTouchingSince(
  ...
  try {
    ...
    throw new Error(`git log ${sinceSha}..HEAD -- ${path} failed: ${errorMessage(e)}`)
  }
}
