 ...
 * sentinel scopes the no-revert-guard bypass to exactly the amend + push.
...
async function main(): Promise<number> {
  ...
  const { newHead } = await squashSingleCommit({ origHead, worktree })
  ...
  await run(
    'git',
    ['push', '--force-with-lease', 'origin', `HEAD:${base}`],
    worktree,
