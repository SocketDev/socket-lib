...
| `./npm/registry`                         | ✅ flagged                | Browser-safe npm registry client — pure parsers + injectable-fetch shell. Zero Node deps; HTTP adapter injected by caller. `browser` condition emitted. Verified by the e2e webpack bundle test.                               |
