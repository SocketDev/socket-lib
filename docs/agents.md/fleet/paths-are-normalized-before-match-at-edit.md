...
| --------- | ------ | ----------------- |
| Lint rule (write-time) | `socket/paths-are-normalized-before-match-at-edit` | Un-normalized path ops in any edited `.ts`/`.mts` source |
| Belt check (commit-time) | `scripts/fleet/check/paths-are-normalized-before-match-at-commit.mts` | Backlog scan of all committed source files |
