...
- `no-clipboard-access-guard` — PreToolUse Bash + Edit/Write: blocks system-clipboard access. Two surfaces: a clipboard CLI in a Bash command (`pbcopy`/`pbpaste`, `xclip`/`xsel`, `wl-copy`/`wl-paste`, `clip`/`clip.exe`), AST-parsed via the fleet shell parser, or source that emits an OSC-52 clipboard escape (`ESC ]52;`). The clipboard is a cross-process exfil + overwrite surface fleet tooling never needs. Bypass `Allow clipboard-access bypass`
