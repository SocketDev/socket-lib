...
A fleet-managed region is delimited by an HTML-element-like marker: an open tag
on the `BEGIN` line and a bare close tag on the `END` line. The `BEGIN`/`END`
keywords are kept so the markers stand out; only the surrounding comment
delimiter changes per file syntax.

| Comment style                                                                             | BEGIN marker                       | END marker                        |
| ----------------------------------------------------------------------------------------- | ---------------------------------- | --------------------------------- |
| HTML (`CLAUDE.md`, `README.md`)                                                           | `<!-- BEGIN <fleet-canonical> -->` | `<!-- END </fleet-canonical> -->` |
| `#` (`.gitignore`, `.gitattributes`, YAML, `.gitmodules`, JSON **array** sentinel string) | `# BEGIN <fleet-canonical>`        | `# END </fleet-canonical>`        |
| `//` (JS/TS, JSON **object** sentinel key)                                                | `// BEGIN <fleet-canonical>`       | `// END </fleet-canonical>`       |
...
- **Nested + balanced** — blocks nest and must be balanced; overlap, an unclosed
  `BEGIN`, or an orphan `END` is **malformed → rejected** (never auto-fixed).
