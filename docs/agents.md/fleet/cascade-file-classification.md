...
| `package.json`                             | `package-scripts`, `package-files`                                              | fleet script names/bodies, `engines`, `packageManager`, catalog devDeps | `name`, `description`, version, repo scripts                                                    |
...
| `.config/fleet/.prettierignore`            | `identical-files`, sentinel-scoped (+ `lockstep-mirror-markers-are-declared`)   | everything through the fleet-canonical end sentinel                     | derived `lockstep-mirrors` block after the end sentinel, appended by `emit-mirror-globs.mts`    |
