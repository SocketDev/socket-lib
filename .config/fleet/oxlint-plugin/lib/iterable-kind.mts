/*
 * @file Shared "is this binding a Set / Map / Iterable?" heuristic used by
 *   no-cached-for-on-iterable AND by prefer-cached-for-loop's skip list.
 *   Without TypeScript type info available to oxlint plugins, the detection is
 *   AST-only:
 *
 *   - `new Set(...)` / `new Map(...)` / `new WeakSet(...)` / `new WeakMap(...)`
 *     initializer → set/map
 *   - `: Set<...>` / `: ReadonlySet<...>` / `: Map<...>` / `: ReadonlyMap<...>` /
 *     `: WeakSet<...>` / `: WeakMap<...>` annotation → set/map
 ...
 *      length } = X; …)` loop is applied to a set / map / iterable.
 *   2. `prefer-cached-for-loop` — needs to SKIP rewriting `for (const item of
 *      setVar)` into the cached-length shape, because doing so produces the
 *      silent-no-op bug the other rule catches. Without this skip, the two
 *      rules race each other and the autofix re-introduces the bug.
 *
 *   # Scope handling
 *
 *   Bindings are resolved by walking the AST `parent` chain from the USE site
 *   upward, stopping at the nearest scope-creating node that declares the name.
 *   A scope-creating node is any of:
 *
 ...
 *     `Map<name, Kind>` populated by visitor side-effect. That model conflated
 *     bindings across scopes — a function-local `const closure = new Map()`
 *     propagated the `map` classification to every other binding in the file
 *     named `closure`, including unrelated arrays in the parent scope. The
 *     scope-walk path fixes that at the cost of a per-lookup walk; rule lookups
 *     happen on `ForStatement` and `MemberExpression` which are relatively
 *     rare, so the overhead is bounded.
 */
