 ...
 *   ...options } as typeof options`; a member-access reader gets a NORMALIZED
 *   LOCAL `const opts = { __proto__: null, ...options } as typeof options`
 *   prepended to the body and each `options.x` read repointed at `opts.x`. The
 *   fix never reassigns the param in place — the fleet bans variable shadowing,
...
const rule = {
  ...
  create(context: RuleContext) {
    ...
    function check(node: AstNode): void {
      ...
      // Fix strategy (both forms cast `as typeof options` so a closed options
      // type rejects the `__proto__` excess property — TS2353 — on the bare
      // spread; the param's own type erases the cast):
      ...
      //     `const opts = { __proto__: null, ...options } as typeof options` as
      //     the first body statement and repoint each `options.x` read at it.
      //     We never reassign the param (`options = …`): the fleet bans variable
      //     shadowing, and an in-place reassign conflates the raw input with its
      //     normalized form (the anti-pattern options-param-naming kills). The
