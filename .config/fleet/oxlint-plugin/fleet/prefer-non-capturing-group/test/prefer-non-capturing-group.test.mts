...
describe('socket/prefer-non-capturing-group', () => {
  test('valid + invalid cases', () => {
    new RuleTester().run('prefer-non-capturing-group', rule, {
      ...
      invalid: [
        {
          name: 'bare alternation in test-only regex (reported, never fixed)',
          code: 'export const r = /\\.(md|mdx)$/\n',
          errors: [{ messageId: 'captureGroup' }],
        },
        {
          name: 'bare alternation, multiple groups (reported, never fixed)',
        ...
        {
          name: 'reading match[0] (whole match) does NOT excuse an unused capture',
          // Blind-spot regression: `m?.[0]` is the whole match, not a
          // group, so it must not silence the rule — the bare (md|mdx)
          // capture is still unused and gets flagged.
          code: [
            'export function f(s: string) {',
            '  const m = /\\.(md|mdx)$/.exec(s)',
            '  return m?.[0]',
            '}',
            '',
          ].join('\n'),
