 ...
 *   MemberExpression and whose FIRST argument is a regex literal that rewrites
 *   path separators. No autofix — the rewrite is contextual (the right shape is
...
const rule = {
  meta: {
    ...
    docs: {
      description:
        'Normalize a path with `normalizePath` from `@socketsecurity/lib/paths/normalize` instead of rewriting separators by hand with a `.replace(...)` over a path-separator regex.',
