 ...
 *   `Buffer.concat([...])` throws `TypeError: list[0] must be a Buffer`. This is
 *   not theoretical — it broke socket-lib's secrets {macos,linux,windows}
 *   backends, which called `.setEncoding('utf8')` on a stream the lib was
 *   concurrently buffering.
 *
 *   The fix is to read the captured output from the AWAITED result instead:
 *   `const { stdout } = await spawn(cmd, args, { stdioString: true })` (wrap in
 *   try/catch — the wrapper rejects on a non-zero exit, the error carrying
 *   `.code` + `.stdout` + `.stderr`). If you genuinely need to stream the
 *   output yourself, pass `stdio` options so the wrapper does not also buffer.
 *
 *   This rule flags ONLY `.setEncoding(...)` on the `.stdout` / `.stderr` of a
 *   fleet `spawn(...)` child reached through `.process` (`const { process } =
 *   spawn(...)` then `process.stdout.setEncoding(...)`, `const c = spawn(...);
 ...
 *   streaming pattern (a hook test streaming stderr does it) and is NOT flagged.
 *   Accessing the stream off the BARE spawn return (`const c = spawn(...);
 *   c.stdout.on(...)`) is a different bug caught by
 *   `socket/no-bare-spawn-childproc-access`. Report-only: the right fix is
 *   contextual (await the wrapper, or pass `stdio` options), so the human picks.
 *   Bypass: a `socket-lint: allow spawn-stream-double-consume` comment on or
