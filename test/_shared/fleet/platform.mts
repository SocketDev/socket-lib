/**
 * @file Fleet-canonical platform predicates and platform-aware name helpers
 *   for tests. Re-exports the single-source-of-truth `WIN32` + `normalizePath`
 ...
 * env-overridden test fixtures, additional separators to fold — flows
 ...
 * elsewhere. Use only when a test asserts on a path string that hasn't
 * been normalized through {@link normalizePath} / `toUnixPath`. Prefer
 * normalizing the value under test instead — this helper covers the rare
 * case where the un-normalized form is the actual contract.
 */
export const NATIVE_PATH_SEP: string = WIN32 ? '\\' : '/'

/**
 * Append `.exe` to `name` on Windows; return `name` unchanged elsewhere.
 * For tests that resolve a binary by basename and need to match the on-disk
