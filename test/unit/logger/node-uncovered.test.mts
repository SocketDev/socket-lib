/**
 * @file Tests for the remaining `Logger` methods in src/logger/node.ts not
 *   covered by node.test.mts / node-advanced.test.mts: done(), progress(),
 *   clearLine(), resetIndent(), logNewline(), skip(), write(), and
 *   errorNewline(). Uses a `CaptureStream` double that tracks TTY-only
 *   behavior (cursorTo/clearLine) so clearLine()'s TTY branch is exercised.
 */
