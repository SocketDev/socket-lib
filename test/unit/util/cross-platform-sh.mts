 ...
 *   shell-free `spawnSync` so behavior is identical across darwin / linux /
 ...
 * lib's shell-free `spawnSync`. Returns the trimmed stdout of the last
 * segment.
 */
export function sh(cwd: string, cmd: string): string {
  ...
  for (let i = 0, { length } = segments; i < length; i += 1) {
    ...
    if (result.status !== 0) {
      throw new Error(
        `sh: command exited with code ${result.status}: ${argv.join(' ')}\n${result.stderr}`,
      )
    }
    out = result.stdout
  }
