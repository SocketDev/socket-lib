 ...
 *
 *   - Downstream validation of length, prefix, and charset can evolve and every
...
/**
 * Canonical fake token for Socket API endpoints. Uses the real `sktsec_` prefix
 * so tests exercise prefix validation if it is added later.
 */
export const FAKE_SOCKET_TOKEN =
  'sktsec_socket-lib-test-fake-token_abc123_XXXXXXXXXXXXXXXXXXXXXXXXXXXX'

/**
 * Canonical fake GitHub personal access token. GitHub PATs begin with `ghp_`
 * and are ~40 chars; `gho_` for OAuth, `ghs_` for apps. This value is
 * structurally valid but contains the test marker for hook allowlisting.
 */
export const FAKE_GITHUB_TOKEN =
  'ghp_socket-lib-test-fake-token_abc123_XXXXXXXXXXXXXXXXXXXXXXXXXXXX'

/**
 * Canonical fake generic bearer token for tests that don't care about a
 * particular provider format.
 */
