/**
 * @file GraphQL-fallback unit tests for GitHub `getReleaseAssetUrl`, split out
 *   from github-api-get-asset-url.test.mts to stay under the line cap.
 */
...
describe(
  'releases/github-api: getReleaseAssetUrl (GraphQL fallback)',
  { concurrent: false },
  () => {
    afterEach(() => {
    ...
    it(
      'should throw when GraphQL fallback returns null release',
      async () => {
        ...
        // rather than a silent skip so the caller knows the tag is
        // the problem, not the transport.
        let call = 0
    ...
    it(
      'should throw when GraphQL fallback returns errors[]',
      async () => {
        ...
        vi.mocked(httpRequest).mockImplementation(async () => {
          ...
          return createMockHttpResponse(
            Buffer.from(
              JSONStringify({
                errors: [
                  { message: 'Field "release" requires authentication' },
                ],
              }),
            ),
            true,
    ...
    it(
      'should retry via pRetry when both REST and GraphQL return empty',
      async () => {
    ...
    it(
      'should throw on GraphQL fallback malformed JSON body',
      async () => {
    ...
    it(
      'should throw on REST per-tag malformed JSON body',
      async () => {
    ...
    it(
      'should throw on REST per-tag release missing assets',
      async () => {
