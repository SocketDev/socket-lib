...
describe(
  'releases/github-api: getLatestRelease (GraphQL fallback)',
  { concurrent: false },
  () => {
    afterEach(() => {
    ...
    it('should fall back to GraphQL when REST returns 200 + empty body', async () => {
      ...
      vi.mocked(httpRequest)
        // 1st call: REST → 200 + empty body, the incident shape
    ...
    it(
      'should propagate errors[] from GraphQL fallback',
      async () => {
        ...
        vi.mocked(httpRequest).mockImplementation(async () => {
          call += 1
          // Odd calls = REST with an empty body, even = GraphQL errors[].
          if (call % 2 === 1) {
    ...
    it('should NOT call GraphQL when REST returns a populated array', async () => {
      ...
      // should NOT fire, so there is no second http call. Verified by
      // setting a single `mockResolvedValueOnce` — if the helper made a
    ...
    it(
      'should throw on REST non-OK status (5xx)',
      async () => {
    ...
    it(
      ...
      async () => {
        // REST returns the incident-shape 200 + empty body → fall back to
        // GraphQL. GraphQL itself returns non-OK. The wrapper surfaces a
        // terse "both REST and GraphQL backends degraded" library-API
        // error so the operator immediately knows to check GitHub status,
        // not their config.
        let call = 0
        vi.mocked(httpRequest).mockImplementation(async () => {
          call += 1
    ...
    it(
      'should throw on GraphQL fallback malformed JSON',
      async () => {
