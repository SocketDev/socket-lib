// vitest specs for sync-registry-workflow-pins pure helpers.
...
import {
  ...
  readLocalPinFromGit,
  reconcilePins,
} from '../../../scripts/fleet/sync-registry-workflow-pins.mts'
...
// gh-aw migration: a delegator may pin the compiled `<workflow>.lock.yml`
// instead of the legacy `<workflow>.yml`. Both forms must match + reconcile.
test('pinLineRe + parseLocalPin tolerate the gh-aw .lock.yml form', () => {
  const lock = `    uses: SocketDev/socket-registry/.github/workflows/weekly-update.lock.yml@${OLD} # main (2026-06-12)`
  assert.equal(pinLineRe('weekly-update').test(lock), true)
  const pin = parseLocalPin('weekly-update', lock)
  assert.equal(pin?.sha, OLD)
  assert.equal(pin?.comment, '# main (2026-06-12)')
  // Still workflow-specific against a .lock.yml line.
  assert.equal(pinLineRe('daily-update').test(lock), false)
})

test('reconcilePins repins a .lock.yml delegator in fix mode', () => {
  const dir = mkdtempSync(path.join(os.tmpdir(), 'repin-'))
  try {
    const f = path.join(dir, '_local-not-for-reuse-weekly-update.yml')
    writeFileSync(
      f,
      [
        'jobs:',
        '  weekly-update:',
        `    uses: SocketDev/socket-registry/.github/workflows/weekly-update.lock.yml@${OLD} # main (2026-06-01)`,
        '',
      ].join('\n'),
    )
    const pins = new Map([
      ['weekly-update', { sha: NEW, comment: '# main (2026-06-12)' }],
    ])
    reconcilePins([f], pins, { fix: true })
    const after = readFileSync(f, 'utf8')
    assert.match(
      after,
      new RegExp(`weekly-update\\.lock\\.yml@${NEW} # main \\(2026-06-12\\)`),
    )
...
test('reconcilePins reports drift without fixing in report mode', () => {
  ...
  try {
    ...
    const drift = reconcilePins([f], pins)
...
test('reconcilePins is a no-op when already at the wanted SHA', () => {
  ...
  try {
    ...
    const pins = new Map([['ci', { sha: NEW, comment: '# main (2026-05-28)' }]])
    assert.equal(reconcilePins([f], pins, { fix: true }).length, 0)
  } finally {
    rmSync(dir, { force: true, recursive: true })
  }
})
...
// Strip inherited git-context env (GIT_DIR/GIT_INDEX_FILE/…) so the temp-repo
// git ops don't operate on the ambient repo when this test runs under a git
// hook (pre-commit exports those, which otherwise corrupts the temp commit).
const TEST_GIT_ENV = gitEnvForOtherRepo()

function git(cwd: string, ...args: string[]): void {
  const r = spawnSync('git', ['-C', cwd, ...args], { env: TEST_GIT_ENV })
