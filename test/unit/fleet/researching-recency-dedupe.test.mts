...
test('normalizeText lowercases, strips punctuation, squeezes whitespace', () => {
  assert.equal(normalizeText('  Rolldown:  the  BUNDLER! '), 'rolldown the bundler')
})
...
test('hybridSimilarity takes the max of trigram and token similarity', () => {
  const reordered = hybridSimilarity('rolldown bundler fast', 'fast bundler rolldown')
  assert.ok(reordered >= 0.9, `reordered text should be near-identical, got ${reordered}`)
...
test('dedupeItems drops a near-duplicate and keeps the earlier item', () => {
  const items = [
    makeItem({ itemId: 'first', title: 'Rolldown is the fastest bundler in 2026' }),
    makeItem({ itemId: 'dup', title: 'Rolldown is the fastest bundler in 2026!' }),
...
test('dedupeItems passes through items with no dedup text', () => {
  const items = [
    makeItem({ itemId: 'a', title: '', body: '', author: undefined, container: undefined }),
    makeItem({ itemId: 'b', title: '', body: '', author: undefined, container: undefined }),
  ]
