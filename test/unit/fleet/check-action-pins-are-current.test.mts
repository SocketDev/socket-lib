...
describe('parseDeclaredDataDeps', () => {
  test('parses the comma list, normalizes, sorts', () => {
    assert.deepEqual(
      parseDeclaredDataDeps('# cascade-data-deps: external-tools.json, .github/actions/lib'),
...
describe('coversRead', () => {
  test('exact match', () => {
    assert.equal(coversRead(['external-tools.json'], 'external-tools.json'), true)
  })
  test('ancestor dir covers a file beneath it', () => {
    assert.equal(coversRead(['.github/actions/lib'], '.github/actions/lib/u.mjs'), true)
  })
...
describe('parseInternalPins', () => {
  test('parses repo/kind/name/sha and strips commented uses', () => {
    ...
    assert.equal(
      parseInternalPins(`# uses: ${SELF}/.github/workflows/ci@${SHA('b')}`).length,
      0,
    )
  })
})
...
describe('closureFor', () => {
  test('includes a transitive self-repo dep + its data edge', () => {
    const units = new Map<string, Unit>([
      ['actions/setup', unit('actions', 'setup', { dataDeps: ['external-tools.json'] })],
      [
  ...
  test('does not follow a cross-repo dep of the same basename', () => {
    const units = new Map<string, Unit>([
      ['workflows/ci', unit('workflows', 'ci', { dataDeps: ['LOCAL_ONLY.txt'] })],
      [
        'workflows/release',
        unit('workflows', 'release', {
          deps: [{ repo: 'Other/repo', kind: 'workflows', name: 'ci', sha: SHA('a') }],
        }),
      ],
    ])
  ...
  test('cycle-guarded', () => {
    const units = new Map<string, Unit>([
      ['actions/a', unit('actions', 'a', { deps: [{ repo: SELF, kind: 'actions', name: 'b', sha: SHA('1') }] })],
      ['actions/b', unit('actions', 'b', { deps: [{ repo: SELF, kind: 'actions', name: 'a', sha: SHA('2') }] })],
    ])
...
describe('findUndeclared', () => {
  test('flags an escaping read missing a data-dep declaration', () => {
    const units = new Map<string, Unit>([
      ['actions/setup', unit('actions', 'setup', { reads: ['external-tools.json'] })],
    ])
...
describe('findStalePins', () => {
  ...
  test('a self-repo pin to a non-local dep is skipped', () => {
    const units = new Map<string, Unit>([
      [
        'workflows/ci',
        unit('workflows', 'ci', {
          deps: [{ repo: SELF, kind: 'actions', name: 'remote-only', sha: SHA('a') }],
        }),
      ],
    ])
    assert.deepEqual(
      findStalePins(units, 'origin/main', fakeGit({}), SELF),
      [],
    )
  })
})
