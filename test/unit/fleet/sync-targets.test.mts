...
describe('path scoping (over-scope guard)', () => {
  ...
  test('resolveTargetLeaves(foundationals) is the six base leaves', () => {
...
describe('scope validation', () => {
  ...
  test('registry-workflows is an override target (no dogfood scope)', () => {
    expect(SYNC_TARGETS['registry-workflows']!.scopes).not.toContain('dogfood')
    expect(SYNC_TARGETS['registry-workflows']!.scopes).toContain('repo')
  })
})
