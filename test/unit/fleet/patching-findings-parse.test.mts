...
import { parsePatchResult, parseReviewResult, renderPatchesMd, summarizeOutcomes, unescapeEntities } from '../../../scripts/fleet/patching-findings/lib/patch-parse.mts'
...
describe('parsePatchResult', () => {
  ...
  test('NONE / empty diff → no_patch', () => {
    ...
    assert.equal(parsePatchResult('<patch_diff></patch_diff>').status, 'no_patch')
  })
})
...
describe('parseReviewResult', () => {
  ...
  test('a low style under REJECT is not a contradiction', () => {
    assert.equal(
      parseReviewResult('REVIEW: REJECT\nSTYLE_SCORE: 2\nOUT_OF_SCOPE_HUNKS: none\nREASON: x')
        .style_contradiction,
      false,
    )
  })
...
describe('summarizeOutcomes + renderPatchesMd', () => {
  const outcomes: PatchOutcome[] = [
    ...
    { applied: false, id: 'f3', skip_reason: 'fp', status: 'no_patch', title: 'skipped one' },
  ]
