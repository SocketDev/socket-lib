...
import { countBySeverity, dedupeFindings, dropRefuted, findingKey, gradeOf, mergeVariants } from '../../../scripts/fleet/scanning-quality/lib/findings.mts'
import type { QualityFinding, RefuteVote } from '../../../scripts/fleet/scanning-quality/lib/findings.mts'
...
describe('findingKey + dedupeFindings', () => {
  test('normalizes the issue text (case + punctuation) for the key', () => {
    assert.equal(findingKey(f({ issue: 'Race Condition!' })), findingKey(f({ issue: 'race condition' })))
  })
...
describe('countBySeverity', () => {
  test('tallies each severity bucket', () => {
    assert.deepEqual(
      countBySeverity([f({ severity: 'critical' }), f({ severity: 'high' }), f({ severity: 'high' })]),
