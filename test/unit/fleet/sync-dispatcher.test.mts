...
describe('findingInScope (all = full payload)', () => {
  ...
  test.each([
    ...
    ['content_drift', '.config/fleet/docker-prebakes.json'],
...
describe('resolveSyncRules validation', () => {
  ...
  test('throws when a target forbids the scope', () => {
    // registry-workflows is fleet/repo only — not dogfood.
    expect(() => resolveSyncRules(['registry-workflows'], 'dogfood')).toThrow(
      /does not support the "dogfood" scope/,
    )
  })
