...
import { assertEveryFindingOnce, buildTriageEnvelope, computeSummary, sortFindings, terminalSummary } from '../../../scripts/fleet/triaging-findings/lib/report.mts'
...
describe('sortFindings', () => {
  test('verdict order: true_positive, then duplicate, then false_positive', () => {
    ...
    assert.deepEqual(out.map(f => f.verdict), [
      'true_positive',
      'duplicate',
      'false_positive',
    ])
  ...
  test('true positives by severity then confidence', () => {
    ...
    assert.deepEqual(out.map(f => f.id), ['hiB', 'hiA', 'low'])
  })
})
