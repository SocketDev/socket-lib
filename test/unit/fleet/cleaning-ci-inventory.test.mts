...
import { findOrphanFiles, isStaleRecord } from '../../../.claude/skills/fleet/cleaning-ci/lib/clean-ci.mts'
...
describe('isStaleRecord', () => {
  ...
  test('orphan-named record is stale even if its file exists', () => {
    assert.equal(
      isStaleRecord(rec({ name: 'Lint', path: '.github/workflows/lint.yml' }), d),
      true,
    )
  })
  ...
  test('a record whose backing file is gone is stale', () => {
    assert.equal(
      isStaleRecord(rec({ name: 'Gone', path: '.github/workflows/deleted.yml' }), d),
      true,
    )
  })
