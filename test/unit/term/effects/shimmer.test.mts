 ...
 *   `(spec, length, frame) → RGB[]`. These tests verify that the kernels,
 *   sweep generators, and palette helpers compose correctly and produce
...
describe('effects/shimmer', () => {
  ...
  describe('configToSpec', () => {
    ...
    it('honors custom highlight color', () => {
      ...
      // Char 0 should be GREEN, the highlight on the wave center.
    ...
    it('honors custom width on smoothKernel', () => {
      ...
      // narrow halfWidth=1 means char 2 is OUTSIDE the bright zone, so it
      // keeps the base color.
      // wide halfWidth=4 means char 2 is INSIDE, so it gets some white blend.
