/**
 * @file Unit tests for the shimmer keyframes adapter.
 */
  ...
  describe('toShimmerKeyframes', () => {
    ...
    it('emits a t=1 keyframe to close the loop', () => {
      ...
      for (const track of tracks) {
        ...
        // Closing value matches the opening value for a clean loop.
        expect(track.values[track.values.length - 1]).toBe(track.values[0])
      }
    })

    it('deduplicates consecutive identical colors', () => {
      // A static spec whose wave never moves should produce just two
      // keyframes per char: the initial value and the t=1 closure.
    ...
    it('agrees with frameColors at every emitted keyframe time', () => {
      // Frame-by-frame correctness: for every keyframe time and value, there
