/**
 * @file Unit tests for object manipulation utilities. Tests advanced object
 *   manipulation and lazy property patterns:
 *
 *   - Lazy getters: createLazyGetter(), defineLazyGetter(), defineLazyGetters()
 *     with memoization
 ...
 *   - Aliases: objectAssign, objectEntries, objectFreeze (direct references to
 *     Object.*)
 *   - Constants: createConstantsObject() for frozen objects with typed getters
 *   - Sorting: entryKeyComparator() for consistent key ordering Tests validate
 *     lazy evaluation, memoization, stats tracking, type narrowing, and edge
 *     cases. Lazy getters are critical for performance - deferring expensive
