...
describe.sequential('dlx-binary', () => {
  describe('getDlxCachePath', () => {
    it('should return normalized cache path', () => {
      ...
      // oxlint-disable-next-line socket/paths-are-normalized-before-match-at-edit -- asserts backslash ABSENCE; normalizing first would make the check vacuous.
  ...
  describe('dlxBinary', () => {
    ...
    // every other test.
    it(
      'should download and cache binary',
      async () => {
    ...
    it('should force re-download when yes option is true (CLI-style)', async () => {
      await runWithTempDir(async tmpDir => {
        ...
        try {
          ...
          // Second call with yes, which should behave like force
          const result = await dlxBinary(['--version'], {
            name: 'yes-binary',
            url,
            yes: true,
