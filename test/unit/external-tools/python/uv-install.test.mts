...
describe(
  'external-tools/python/uv-install — uvSyncProject',
  { concurrent: false },
  () => {
...
describe(
  'external-tools/python/uv-install — uvExportMaterialize',
  { concurrent: false },
  () => {
    ...
    test('a live lock holder is polled until the target appears', async () => {
      ...
      try {
        ...
        readdirMock.mockResolvedValueOnce([]).mockResolvedValueOnce(['pkg'])
        writeFileMock.mockRejectedValueOnce(eexist())
        // This process's PID is live → isStaleLock false → enter the poll loop.
