...
describe(
  'external-tools/trivy/resolve — doResolveTrivy',
  { concurrent: false },
  () => {
...
describe(
  'external-tools/trivy/resolve — resolveTrivy memoization',
  { concurrent: false },
  () => {
    ...
    test('uses separate cache slots for different option shapes', async () => {
      const { fromDownload, fromPath, fromVfs, resolveTrivy } =
        await loadFresh()
      fromVfs.mockResolvedValue(undefined)
