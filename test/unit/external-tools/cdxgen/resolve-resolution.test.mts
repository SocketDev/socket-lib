...
describe(
  'external-tools/cdxgen/resolve — doResolveCdxgen',
  { concurrent: false },
  () => {
...
describe(
  'external-tools/cdxgen/resolve — resolveCdxgen memoization',
  { concurrent: false },
  () => {
    ...
    test('uses separate cache slots for different option shapes', async () => {
      const { fromDownload, fromPath, fromVfs, resolveCdxgen } =
        await loadFresh()
      fromVfs.mockResolvedValue(undefined)
