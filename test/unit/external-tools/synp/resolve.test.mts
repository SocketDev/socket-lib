...
describe(
  'external-tools/synp/resolve — cacheKey',
  { concurrent: false },
  () => {
...
describe(
  'external-tools/synp/resolve — doResolveSynp',
  { concurrent: false },
  () => {
    ...
    test('falls through to download when PATH misses and opts.downloadIfMissing is set', async () => {
      const { doResolveSynp, fromDownload, fromPath, fromVfs } =
        await loadFresh()
      fromVfs.mockResolvedValueOnce(undefined)
      fromPath.mockResolvedValueOnce(undefined)
      const expected = {
        binaryPath: '/cache/synp',
...
describe(
  'external-tools/synp/resolve — resolveSynp memoization',
  { concurrent: false },
  () => {
