...
describe(
  'external-tools/janus/resolve — doResolveJanus',
  { concurrent: false },
  () => {
...
describe(
  'external-tools/janus/resolve — resolveJanus memoization',
  { concurrent: false },
  () => {
    ...
    test('uses separate cache slots for different option shapes', async () => {
      const { fromDownload, fromPath, fromVfs, resolveJanus } =
        await loadFresh()
      fromVfs.mockResolvedValue(undefined)
