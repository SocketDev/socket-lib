...
// A plain `function` spy is `new`-able where an arrow function is not, and, by
...
describe('snapshot safety — lazy abort-signal acquisition', () => {
  ...
  // module, asserting that import alone is signal-free, then runs its using
...
describe('snapshot safety — built dist survives node --build-snapshot', () => {
  // Ground truth: snapshot the REAL built CJS module, never a src spy. A pinned
  ...
  // an unrelated primordials concern that is out of scope here.
  ...
  for (let i = 0, { length } = modules; i < length; i += 1) {
    ...
    it.skipIf(!hasDist || isWindows)(
      ...
      () => {
        ...
        // Neutralize inherited coverage collection: `node --build-snapshot`
        // SEGFAULTS when NODE_V8_COVERAGE points at a directory (the cover
        // tier exports it to every child), and coverage of a snapshot-builder
        // child is meaningless. The spawn wrapper MERGES env over process.env,
        // so deleted keys re-inject — override with empty strings instead
        // (empty disables collection; verified: dir => exit 139, '' => 0).
