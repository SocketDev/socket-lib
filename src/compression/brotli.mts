 ...
 *   raw-stream variants. Default quality is 11 for maximum compression at the
 *   cost of speed, on the assumption these are one-shot CLI calls. Override
 *   via `options.level` for hot paths. await
 *   compressBrotli(JSON.stringify(payload)) await
...
import { resolveFileArgs, stripExt } from './shared.mjs'
