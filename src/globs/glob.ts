/**
 * @fileoverview `glob` (async) and `globSync` — fast-glob wrappers
 * with a `node:fs.glob` fast-path when the option surface lines up.
 * `canUseNodeFsGlob` is the per-call gate.
 *
 ...
 *
 * fast-glob has TWO independent filters that handle the trailing slash
 * differently:
 *
 *   1. The DEEP filter decides whether to walk INTO a candidate
 *      directory. The deep filter compiles `**\/dist/` into a regex
 *      that requires a trailing slash on the input, but it tests
 *      `entryPath = 'dist'` (no slash, because readdir entries don't
 *      include one). So fast-glob walks in anyway.
 *
 *   2. The ENTRY filter (post-walk) retries with a trailing slash
 *      appended for directory entries — so it correctly excludes the
 *      results, but only AFTER the entire subtree has been walked.
 *
 * Net effect: a `dist/` ignore pattern correctly removes contents from
 * the result array, but only after walking. On a 300k-file `dist/`
 * under tight memory this is the difference between "instant" and
 * "OOM kill". Stripping the trailing slash makes it `**\/dist`, which
 * both filters interpret correctly.
 */
