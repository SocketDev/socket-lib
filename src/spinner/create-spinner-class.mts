 ...
 *   instead of at module scope preserves the spinner's lazy-init guarantee
 *   while splitting the bulk of the class body out of the factory module.
 */

import type { ColorRgb, ColorValue } from '../term/colors/types.mjs'
...
import {
  ...
  lastWasBlankSymbol,
  LOG_SYMBOLS,
} from '../logger/symbols.mjs'
import { ArrayPrototypeAt, ArrayPrototypeSlice } from '../primordials/array.mjs'
import { MathMax } from '../primordials/math.mjs'
import { ObjectDefineProperties } from '../primordials/object.mjs'
import { isBlankString } from '../strings/predicates.mjs'
import { stringWidth } from '../strings/width.mjs'

import { desc, formatProgress, normalizeText } from './format.mjs'
import {
  applyShimmer,
  parseShimmerOption,
  resolveSpinnerColorRgb,
} from './spinner-internals.mjs'
import {
  ...
  setShimmerSymbol,
  updateTextSymbol,
} from './spinner-shimmer-methods.mjs'
import {
  applyStatusSymbol,
  installStatusMethods,
  showStatusSymbol,
} from './spinner-status-methods.mjs'
...
export type YoctoSpinnerConstructor = new (...args: unknown[]) => {
  ...
  // signature, so internal writes use bracket access via `super['text']`.
  isSpinning: boolean
  [key: string]: unknown
}
...
export function createSpinnerClass(
  ...
  const SpinnerCtor = class SpinnerClass extends YoctoSpinnerClass {
     ...
     *
     * @param value - Text to display; omit to get the current text.
     *
