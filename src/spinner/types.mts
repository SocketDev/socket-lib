...
export type ShimmerInfo = {
  /**
   * User-facing color reference: inherit, an explicit value, or a palette.
   */
  color: ColorInherit | ColorValue | Palette
  /**
   * Current direction, driven by config and snapshotted here for getters.
   */
...
export type SpinnerInstance = {
  ...
  /**
   * Show debug message without stopping, only when debug mode is enabled.
   */
  debug(text?: string | undefined, ...extras: unknown[]): SpinnerInstance
  /**
   * Show debug message and stop the spinner, only when debug mode is enabled.
   */
  ...
  /**
   * Disable shimmer effect, preserving config for later re-enable.
   */
  ...
  /**
   * Enable shimmer effect, restoring saved config or using defaults.
   */
