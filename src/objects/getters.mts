 ...
 *   building blocks; `createLazyGetter` is the memoizing primitive. Cycle:
 *   `defineLazyGetters` → `defineLazyGetter` → `defineGetter` +
 *   `createLazyGetter`. All function-only references, no eager top- level
 *   evaluation between siblings, so ESM tolerates.
 */

import { kInternalsSymbol } from '../constants/sentinels.mjs'
 ...
 *
 * This function creates an immutable object with: - Regular properties from
 * `props` - Lazy getters that compute values on first access - Internal
 * properties accessible via `kInternalsSymbol` - Mixin properties (lower
 * priority, won't override existing) - Alphabetically sorted keys for
 * consistency.
 *
 ...
 *
 * The returned function will only call the getter once, caching the result for
 * subsequent calls. This is useful for expensive computations or operations
 * that should only happen when needed.
 *
 * @example
 *   ;```ts
 ...
 *   ```
 *
 * @param name - The property key name for the getter (used for debugging and
 *   stats)
 ...
 *
 * The getter is non-enumerable and configurable, meaning it won't show up in
 * `for...in` loops or `Object.keys()`, but can be redefined later.
 *
 * @example
 *   ;```ts
 *   const obj = {}
 ...
 *
 * @returns The modified object, returned for chaining.
 */
 ...
 *
 * Each getter in the provided object will be converted to a lazy getter and
 * attached to the target object. All getters share the same stats object for
