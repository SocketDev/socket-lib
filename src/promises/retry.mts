...
import { UNDEFINED_TOKEN } from '../constants/sentinels.mjs'
import { MathFloor, MathMin, MathRandom } from '../primordials/math.mjs'
import { getTimers } from './shared.mjs'
import { normalizeRetryOptions } from './options.mjs'

import type { RetryOptions } from './types.mjs'
...
export async function pRetry<T>(
  ...
  async function waitUntilNextAttempt(waitTime: number): Promise<boolean> {
    try {
  ...
  while (attempts-- >= 0) {
    ...
    try {
      ...
      const selectedWait = selectRetryWait(e, attempts, waitTime)
      if (selectedWait === false) {
        break
      }
      waitTime = selectedWait

      if (!(await waitUntilNextAttempt(waitTime))) {
        return undefined
      }
