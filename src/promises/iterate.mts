...
import { arrayChunk } from '../arrays/chunk.mjs'
import { PromiseAllSettled } from '../primordials/promise.mjs'
import { normalizeIterationOptions, normalizeRetryOptions } from './options.mjs'
import { pRetry } from './retry.mjs'

import type { IterationOptions, RetryOptions } from './types.mjs'
...
export async function pFilterChunk<T>(
  ...
  for (let i = 0; i < length; i += 1) {
    ...
    if (signal?.aborted) {
      ...
      filteredChunks[i] = chunk.filter(
        (_value, index) => predicateResults[index],
      )
    }
  }
