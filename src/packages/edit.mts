/**
 * @file Editable package.json manipulation utilities. Convenience wrappers
 *   around the `EditablePackageJson` class. Both the in-memory and path-backed
 *   flows funnel through the same two entry points — pass `path` to persist,
 *   omit it to stay in memory:
 *
 *   - `EditablePackageJsonInstance` — public interface for instances
 *   - `toEditablePackageJson` — async (in-memory if no `path`, loads from disk if
 *     `path` set)
 *   - `toEditablePackageJsonSync` — sync version of the above The class factory
 *     lives in a sibling leaf and is re-exported here so existing
 *     `packages/edit` importers keep working unchanged:
 *   - `getEditablePackageJsonClass` — `./edit-class` Lazy `node:fs` / `node:path`
 *     / `node:util` loaders use the canonical `getNodeFs` / `getNodePath` /
 *     `getNodeUtil` helpers from `@socketsecurity/lib/node/{fs,path,util}`.
 */

import { JSONStringify } from '../primordials/json.mjs'

import { isNodeModules } from '../paths/normalize.mjs'
import { resolvePackageJsonDirname } from '../paths/packages.mjs'

import { getEditablePackageJsonClass } from './edit-class.mjs'
import { normalizePackageJson } from './normalize.mjs'
...
/**
 * EditablePackageJson instance interface extending NPMCliPackageJson
 * functionality. Provides enhanced package.json manipulation with
...
export { getEditablePackageJsonClass } from './edit-class.mjs'
