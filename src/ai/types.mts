 ...
 * timeout, addDirs).
 *
 * Why the lockdown fields are required rather than defaulted to a permissive
 * shape: the CLAUDE.md rule says "all four lockdown flags MUST be set on every
 * spawn." Making them required at the type level is the type-system version of
 * that rule.
 */
export interface SpawnAiAgentOptions {
   ...
   * inherited env. Gated off by default and inherit-only when absent. Used by
 ...
 * - `never` — never auto-remove. The caller or a human inspector cleans up.
