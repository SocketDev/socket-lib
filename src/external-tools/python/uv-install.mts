 ...
 *   uv, Astral's Python package manager, drives a uv PROJECT — a directory
 *   with `pyproject.toml` + `uv.lock` (+ optional `[tool.uv] exclude-newer`).
 *   The lock manifests every transitive version, so the project is the most
...
import { generateCacheKey } from '../../dlx/cache.mjs'

import { isAlreadyInstalled, isStaleLock } from './pip-install.mjs'
 ...
 * holds plain files and no venv, so it is relocatable and embeddable in a smol
 ...
 * lock-vs-manifest drift into a hard non-zero exit rather than a silent
 * re-resolution — this is what makes it verification-grade. Throws when uv
 * exits non-zero on drift, a missing lock, or a resolution failure.
 */
