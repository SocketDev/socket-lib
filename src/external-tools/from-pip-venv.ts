 ...
 *   compose their fourth tier on top of these helpers.
 *
 *   Two helpers:
 *
 ...
 *   - `findPython` — locates `python3` (or `python` on Windows) on PATH.
 *     Returns the absolute path or `undefined` when no Python is available.
 *
 ...
 *   - Verify package integrity beyond pip's own wheel-hash mechanism. Pinning
 *     is the caller's responsibility: pass exact-version `<pkg>==<ver>` or
 *     git-SHA `git+...@<sha>`.
 *   - Re-resolve when the cache is stale. The cache key is derived by the
 *     caller from the install spec; a re-pin produces a new dir.
 */
...
export interface CreatePipVenvOptions {
  ...
  /**
   * Name of the entry-point executable. Matches the package's `[project.scripts]`
   * key (or the package name when it's the same).
   */
  ...
  /**
   * Optional override for the Python interpreter. Defaults to {@link findPython}.
   */
...
export interface CreatePipVenvResult {
  ...
  /**
   * `true` when this call created the venv (and ran pip install); `false`
   * when the existing cache was reused.
   */
  readonly created: boolean
}

/**
 * Create (or reuse) a venv at `cacheDir` and pip-install `installSpec` into
 * it. Returns the entry-point path + a `created` flag. Throws when:
 *
 *   - No Python interpreter is on PATH (and none was passed via `python`).
 *   - `python -m venv` fails.
 *   - `pip install` fails.
 *   - The install succeeded but the entry-point was not created (caller passed
 *     a wrong `entryPoint` name, or the package has no console script).
 */
