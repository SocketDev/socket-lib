/**
 * @fileoverview `bazelFromDownload()` — fetches the upstream Bazel
 * binary and returns a `ResolvedBazel` pointing at the cached
 * executable.
 *
 * Bazel ships as a single executable per platform (no tar/zip
 * wrapper), so this tier uses `downloadToolArchive` directly — no
 * extraction step. The dlx downloader already chmods the file to
 * 0o755 on POSIX, so the cached path is runnable as soon as it
 * lands on disk.
 *
 * Returns `undefined` when Bazel doesn't publish a build for the
 * target `platformArch` (no entry in `BAZEL_ASSET_MAP`).
 *
 * Trust-on-first-use: pass `integrity` from `external-tools.json`
 * when available. Omit on first install; the underlying call
 * computes the SRI and writes it into the dlx cache metadata so a
 * follow-up `external-tools.json` update can pin it.
 */
...
/**
 * Resolve Bazel by downloading the upstream binary. Returns the
 * standard `ResolvedBazel` shape with `source: 'download'`.
 *
 * @example
 * ```typescript
