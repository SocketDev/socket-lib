...
import { getRepoUrlDetails } from '../packages/specs'
 ...
 *   `repository.url`).
 * @param n - Pull request, issue, or stack number.
 * @param kind - `'pull'` for a PR link (default), `'issues'` for an issue
 *   link, or `'stack'` for a stacked-PR link.
 *
 * @returns A Markdown link `[#N](…)` (or `[stack #N](…)` for stacks), or the
 *   bare `#N` when `repoUrl` is unparseable.
 */
export function githubRefLink(
  repoUrl: string,
  n: number,
  kind: 'pull' | 'issues' | 'stack' = 'pull',
): string {
  ...
  if (kind === 'stack') {
    ...
    // against a live stack — all 404; the stack API object carries `url`
    // but no `html_url`), so the REST URL is the canonical resolvable
    // surface. If GitHub ships a stacks UI page, point this at it instead.
    return `[stack #${n}](https://api.github.com/repos/${user}/${project}/stacks/${n})`
  }
