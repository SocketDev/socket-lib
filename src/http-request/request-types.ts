/**
 * @file Types for HTTP request configuration — options, hooks, and Node
 *   `IncomingMessage` aliases. Split out of `http-request/types.ts` for size
 *   hygiene.
 *
 *   - `IncomingResponse` / `IncomingRequest` — Node IncomingMessage aliases
 *   - `HttpHookRequestInfo` / `HttpHookResponseInfo` / `HttpHooks` —
 *     observability
...
/**
 * IncomingMessage received as a request in a server handler (http.createServer
 * callback).
 */
...
export interface HttpRequestOptions {
  /**
   * Request body to send. Can be a string, Buffer, or Readable stream.
   *
   ...
   * certificates are combined with the default trust store via an HTTPS agent.
   * Useful when SSL_CERT_FILE is set but NODE_EXTRA_CA_CERTS was not available
   * at process startup.
   *
   * @example
   *   ;```ts
  ...
  /**
   * Lifecycle hooks for observing request/response events. Hooks fire
   * per-attempt — retries and redirects each trigger separate hook calls.
   */
  ...
  /**
   * Callback invoked before each retry attempt. Allows customizing retry
   * behavior per-attempt (e.g., skip 4xx, honor Retry-After).
   *
   * @param attempt - Current retry attempt number (1-based)
   * @param error - The error that triggered the retry (HttpResponseError for
   *   HTTP errors)
   * @param delay - The calculated delay in ms before next retry.
   *
   * @returns `false` to stop retrying and rethrow, a `number` to override the
   *   delay (ms), or `undefined` to use the calculated delay.
   */
  onRetry?:
    | ((
        attempt: number,
   ...
   *
   * `body`, `text()`, `json()`, and `arrayBuffer()` return empty/zero values
   * since the stream has not been read.
   *
   * Incompatible with `maxResponseSize` (size enforcement requires reading the
   * body).
   *
   * @default false
   */
  ...
  /**
   * When true, non-2xx HTTP responses throw an `HttpResponseError` instead of
   * resolving with `response.ok === false`. This makes HTTP error responses
   * eligible for retry via the `retries` option.
   *
   * @default false
   */
  throwOnError?: boolean | undefined
  /**
   * Request timeout in milliseconds. If the request takes longer than this, it
   * will be aborted.
   *
   * @default 30000
   */
