 ...
 *   refuses to short-circuit on the first mismatched byte.
 *
 *   Why this matters:
 *
 *     - `===` / `!==` on JS strings short-circuits at the first byte mismatch.
 *       An attacker who can measure server response time can binary-search
 *       the secret one byte at a time: `'a000...'`, `'b000...'`, … until the
 *       response slows down at the right first byte, then on to byte 2.
 *       Same trap for `Buffer.compare` and `==`.
 ...
 *       carries no information about which byte mismatched.
 *
 *   Use whenever comparing two values that include a secret (session token,
 *   API key, MAC, expected-hash). Don't use for path strings or other
 *   non-secret comparisons — `===` is fine there and faster.
 *
 *   Patterned after pilcrow's `crypto.go::constantTimeCompare`, the canonical
 *   shape in passwordless-example.auth.pilcrowonpaper.com — wrap once, use
 ...
 * differ. Never throws.
 *
 * Length mismatch handling: `timingSafeEqual` itself throws on length
 * mismatch (it can't preserve the timing-safety contract across differently-
 * sized buffers). We catch that and return `false` so callers don't need
 * a length pre-check.
 *
 * @example
 *   ;```typescript
