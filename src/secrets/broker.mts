/**
 * @file Sockeye broker client. Asks the running sockeye credential daemon for a
 ...
 *   keychain read in `secrets/find.ts` `resolve()`. The broker is OPTIONAL and
 *   self-gating: when the daemon isn't running (no socket file),
 *   `requestFromBroker` returns `undefined` immediately and the caller falls
 *   through to the keychain. It never throws — a broker miss is a fall-through,
 *   not an error. The daemon binary itself is built by socket-btm
 *   (`packages/sockeye-daemon-builder`); download-and-start is a later layer,
...
const SOCKEYE_SOCKET_NAME = 'sockeye'
...
/**
 * Ask the sockeye daemon for a credential. Returns the value on success, or
