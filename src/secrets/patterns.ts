/**
 * @file Shared catalog of secret-bearing env-var key names and literal
 *   secret-value shapes. One source of truth for every scanner that looks for
 *   accidentally-checked-in or accidentally-printed credentials, so a new
 *   vendor shape is added once and every gate picks it up.
 *   Keep the catalog narrow + auditable. Adding a name here means every
 *   consumer will scan for it; false-positives on legitimate config keys
 *   (e.g. `FOO_API_VERSION=2.1`) are real friction. Names follow the published
 *   env-var convention of each tool — when in doubt, prefer the official docs
 *   over guessed shapes. Layout:
 *
 ...
 */
export const GENERIC_TOKEN_SUFFIX_RE: RegExp =
  /^[A-Z_]*(?:ACCESS|API|AUTH|BOT|CLIENT|PRIVATE|SECRET|SESSION|WEBHOOK)_(?:KEY|SECRET|TOKEN)$/

/**
 * Check if the given key name matches any pattern in
 ...
 * @example
 *   ;```ts
 *   isTokenKey('ANTHROPIC_API_KEY') // true
 *   isTokenKey('FOO_API_VERSION') // false
 *   ```
 *
 * @param key - The env-var key name to test.
 *
 * @returns `true` if the key names a known secret-bearing env var.
 */
...
/**
 * Substring fragments matched case-insensitively against arbitrary command
 * text. Different shape from `ALL_TOKEN_KEY_PATTERNS`: those match a parsed
 * KEY= identifier exactly, these match anywhere in arbitrary command text
 * (`curl -H "Authorization: $TOKEN"` → matches "TOKEN" → flag for inspection).
 *
 * Kept short to minimize false positives. A "PASSWORD" mention in a
 * commit-message body would otherwise trip every commit, so consumers should
 * narrow matches to assignment / flag-value positions rather than any
...
// single source of truth shared by Bash-time, edit-time, and commit-time
// scanners — one catalog so a new vendor shape is added once and every gate
// picks it up (code is law, DRY).
...
/**
 * Return the first secret-VALUE shape matched in `text`, or `undefined`. Used
 * by every secret gate (Bash / edit / commit) so they share one detection
 * list.
 *
 * @param text - Arbitrary text to scan for literal secret-value shapes.
 *
 * @returns The first hit's `label` + `match`, or `undefined` when clean.
 */
