...
import { stripAnsi } from '../term/ansi/strip.mjs'
import { IntlSegmenter } from '../primordials/intl.mjs'
...
import type { eastAsianWidth as eastAsianWidthType } from '../external/get-east-asian-width.js'
...
// constructing it at import time pins that handle into every module
// transitively importing this leaf — aborting V8 --build-snapshot
// serialization. The memoized getter keeps the create-once/reuse performance
// win while staying snapshot-safe (mirrors the lazy getEastAsianWidth()
// accessor above).
...
// \p{Extended_Pictographic}, which is broader and slightly less precise.
...
export function stringWidth(text: string): number {
  ...
  for (const { segment } of getSegmenter().segment(plainText)) {
    // Skip zero-width and non-printing clusters such as controls and
    // combining marks.
    ...
    // These are spacing characters rather than combining marks, so they add
