 ...
 *   - `privateConstructorArgs` is deleted after first `Console` build so we don't
 *     pin the original args for the lifetime of the logger.
 */

import { WeakMapCtor } from '../primordials/map-set'
/**
 * The global `console` reference captured at module load. Pinned to a local so
 * dynamic `console` overrides at runtime can't affect the logger's source
 * console (e.g., test rewiring of `globalThis.console`).
 */
export const globalConsole = console

/**
 * Property-descriptor template used when registering dynamic console methods on
 * `Logger.prototype`. Writable + configurable so tests can override;
 ...
 * allows logger to be imported during early Node.js bootstrap before stdout is
 * ready, avoiding ERR_CONSOLE_WRITABLE_STREAM errors.
 */
 ...
 * Private fields can't be accessed from dynamically created functions 2. Logger
 * adds console methods dynamically to its prototype 3. These dynamic methods
 * need constructor args for lazy initialization 4. WeakMap allows both regular
 * methods and dynamic functions to access args.
 *
