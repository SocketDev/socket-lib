 ...
 *   Indentation is tracked at the prefix layer as a per-stream string of
 *   spaces rather than via `node:console`'s frozen `Symbol(kGroupIndent)`, so
 *   these helpers read and write that prefix through a small accessor context
 *   handed in by the calling `Logger`. Pulling these out of `./node` keeps the
...
import { maxIndentation } from './shared.mjs'
import {
...
import type { LoggerTrackable } from './console-methods.mjs'
