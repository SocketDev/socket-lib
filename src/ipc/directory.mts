...
export async function ensureIpcDirectory(filePath: string): Promise<void> {
  ...
  /* c8 ignore stop */
  // Need lstat to discriminate symlink vs dir via isDirectory().
  // oxlint-disable-next-line socket/prefer-exists-sync -- need lstat
  ...
  // Permission bits only; mask out the file-type bits. Reject any group or
