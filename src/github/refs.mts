/**
 * @file Resolve GitHub git refs (tag / branch / commit) to full commit SHAs.
 *   Two-tier strategy with cross-backend fallback:
 *
 *   1. REST cascade (`fetchRefSha`): tag → branch → commit. The first 200 OK wins.
 *   2. GraphQL fallback (`fetchRefShaViaGraphQL`): fires only when the REST
 *      cascade hits the documented 200-OK-empty-body incident shape. GraphQL
 *      queries hit a different backend at GitHub, so it stays consistent
 *      through Elasticsearch outages that produce empty REST bodies. Caching: a
 *      `TtlCache` keyed by `${owner}/${repo}@${ref}` with a 5-minute TTL plus
 *      in-memory memoization. `clearRefCache()` clears the in-memory tier; the
 *      persistent disk tier survives so the cache warms quickly on the next
 *      run. Disable everything with the `DISABLE_GITHUB_CACHE` env var. Module
 *      shape: this file holds the public `resolveRefToSha` entry point.
 *      Supporting surface lives in sibling leaves and is re-exported here so
 *      existing `github/refs` importers keep working unchanged:
 *
 ...
 *   const sha = await resolveRefToSha('owner', 'repo', 'develop', {
...
export { clearRefCache, getGithubCache } from './refs-cache.mjs'
export { fetchRefShaViaGraphQL } from './refs-graphql.mjs'
export { fetchRefSha } from './refs-rest.mjs'
