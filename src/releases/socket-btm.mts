...
import { getArch, getOs } from '../constants/platform.mjs'
import type { Arch, Libc, Platform } from '../constants/platform.mjs'
import { getReleaseAssetUrl } from './github-asset-url.mjs'
import { getLatestRelease } from './github-listing.mjs'
import { downloadGitHubRelease } from './github-downloads.mjs'
import {
  getBinaryAssetName,
  getBinaryName,
  getNodePrebuildAssetName,
  getPlatformArch,
} from './socket-btm-binary-naming.mjs'
...
export interface SocketBtmBinaryConfig {
  ...
  tag?: string | undefined
  /**
   * Target architecture. Defaults to the current arch.
   */
...
/**
 * Configuration for downloading a socket-btm release binary or asset.
 */
 ...
 *   ```
 *
 * @returns 'musl', 'glibc', or undefined on non-Linux platforms
 */
export function detectLibc(): Libc | undefined {
  ...
  try {
    ...
    // If detection fails, default to glibc, the most common variant.
    return 'glibc'
  }
  /* c8 ignore stop */
}
