...
import { DateParse } from '../primordials/date.mjs'
...
import { createAssetMatcher } from './github-assets.mjs'
  ...
  return (
    (await pRetry(async () => {
      ...
      const matchingReleases = releases.filter(release => {
        ...
        // Skip empty releases, the ones with no assets.
      ...
      // Sort by published_at descending so the newest release comes first.
 ...
 * Buffer / string on older paths, OR — depending on the response's
 * content-type handling — an already-parsed object/array. Calling
 * `body.toString('utf8')` + JSON.parse on an already-parsed object yields
 * `"[object Object]"` and throws (the "Failed to parse … releases response"
 * build failure). Pass through non-string/Buffer bodies; parse the rest.
 * Throws on genuine parse failure.
 */
export function parseResponseBody(body: unknown): unknown {
  if (body !== null && typeof body === 'object' && !Buffer.isBuffer(body)) {
    // The http layer already parsed this into an object or array — use as-is.
    return body
  }
