/**
 * @file Pre-flight readability check for file lists. Used to filter out paths
 *   that exist in glob results but cannot be opened — Yarn Berry PnP virtual
 *   filesystem entries, pnpm symlinks pointing into a missing
 *   content-addressable store, or filesystem races during CI runs.
 */

import { canRead } from './access.mjs'

import type { ValidateFilesResult } from './types.mjs'

/**
 * Validate that file paths are readable before processing. Filters out files
 * from glob results that cannot be accessed (common with Yarn Berry PnP virtual
 * filesystem, pnpm content-addressable store symlinks, or filesystem race
 * conditions in CI/CD environments).
 *
