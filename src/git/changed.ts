/**
 * @file "Anything different from HEAD" helpers — the broad-strokes `git status
 *   --porcelain` view that lumps staged, unstaged, and untracked paths
 *   together. Use these when you want a single answer to "did anything
 ...
 *
 * Uses `git status --porcelain` which returns the full working tree status with
 * status codes: - `M` - Modified - `A` - Added - `D` - Deleted - `??` -
 * Untracked - `R` - Renamed - `C` - Copied.
 *
 * This is the most comprehensive check - captures everything that differs from
 * the last commit, including: - Files modified and staged with `git add` -
 * Files modified but not staged - New files not yet tracked by git.
 *
 ...
 * which returns the full working tree status with status codes: - `M` -
 * Modified - `A` - Added - `D` - Deleted - `??` - Untracked - `R` - Renamed -
 * `C` - Copied.
 *
 * This is the most comprehensive check - captures everything that differs from
 * the last commit, including: - Files modified and staged with `git add` -
 ...
 *
 * Checks if the given pathname has any changes including: - Staged
 * modifications (added with `git add`) - Unstaged modifications (not yet
 * staged) - Untracked status (new file/directory not in git)
 *
 ...
 *   const changed = await isChanged('/path/to/repo/src/foo.ts', {
 *     cwd: '/path/to/repo',
 *   })
 *   ```
 *
 ...
 * modifications - Untracked status (new file/directory not in git)
 *
 * For directories, returns `true` if ANY file within the directory has changes.
 *
 * Symlinks in the pathname and cwd are automatically resolved using
 ...
 *
 *   // Check from different cwd
 *   const changed = isChangedSync('/path/to/repo/src/foo.ts', {
 *     cwd: '/path/to/repo',
 *   })
 *   ```
 *
