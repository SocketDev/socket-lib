 ...
 *   created under os.tmpdir(), seeded from HEAD's tree (`git read-tree HEAD`)
 *   so it starts as a copy of the current commit rather than empty, set via
 *   the GIT_INDEX_FILE env var for the duration of the function, and removed
 *   afterward. Seeding matters: an empty temp index means an ordinary `git
 *   add <path>` + `git commit` inside the callback produces a commit whose
 ...
 *   normal commit does: HEAD's tree plus the staged change. On an unborn
 *   branch (no commits yet) `read-tree HEAD` has nothing to copy and the temp
 *   index is left empty, which is correct for a repo with no history. Use
 *   this when a git add/commit races another session's git operations (the
 *   fleet parallel-claude-sessions scenario): the temp index sidesteps the
 *   shared lock entirely, so the commit lands without waiting for the other
 *   process. Note this isolates from the SHARED index's current staged
 *   state too — anything staged-but-uncommitted in the real index is
 *   intentionally not visible inside the callback, which is the point: it
 *   avoids racing whatever that other process has staged. The function MUST
 *   be synchronous: the env mutation is process-global, and an async
 *   function would leak GIT_INDEX_FILE to concurrent microtasks.
 */
...
export interface WithIsolatedIndexOptions {
   ...
   * explicitly to target a repo other than process.cwd() (e.g. from a test,
   * where `process.chdir()` is banned fleet-wide).
   */
 ...
 * directory is created under os.tmpdir(), seeded from HEAD's tree so it
 * starts as a copy of the current commit rather than empty, the env var is
 * set for the duration of the function, and both the directory and the env
 * var are cleaned up in a finally block. The function MUST be synchronous.
 *
 * @example
 *   withIsolatedIndex(() => {
