 ...
 *
 *   1. If the input looks like an absolute or relative path, `resolveRealBinSync`
...
import { binPathCache, getFs } from './shared.mjs'
import { resolveRealBinSync } from './path/resolve.mjs'
import { whichReal } from './path/which.mjs'
...
export async function execBin(
  ...
  // Resolve the binary path, using cache for binary names but not paths.
