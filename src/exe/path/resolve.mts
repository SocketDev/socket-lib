  ...
  if (isWin32()) {
    ...
    if (
      ...
      if (isNpmOrNpx) {
        ...
        // "bin.PS1" from npm cmd-shim v7.0.0. The regex below matches that
        // exact generated text, so an upstream wording change makes it return
        // an empty path rather than fail loudly.
        // Verbatim shim: docs/references/repo/cmd-shim-formats.md
        relPath = /(?<="\$basedir\/).*(?=" $args\n)/.exec(source)?.[0] || ''
      }
      if (relPath) {
