...
/**
 * Whether a blocking wait is available in this context.
 *
 * Node always defines `SharedArrayBuffer`, and `Atomics.wait` is legal on
 * Node's main thread, so this is true for every runtime that can reach
 * `spawnSync` at all. It is false only where the global is absent: a browser
 * that is not cross-origin isolated, and V8's `--build-snapshot` builder.
 * Neither spawns child processes, so the false branch is unreachable from a
 * spawn - it exists so the module stays honest about the dependency rather
 * than assuming a global.
 */
...
/**
 * Block the calling thread for `ms`, without spinning the CPU. Returns whether
 * the wait actually happened.
 *
 * `spawnSync` is synchronous end to end, so a caller that could await a timer
 * would be using `spawn` instead.
 *
 * A caller that cannot wait must NOT retry. Skipping the delay and going
 * straight back at a failing command turns a paced retry into a hot loop
 * against whatever just timed out, which is worse than making one attempt.
 */
