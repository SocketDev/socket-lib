/**
 * @file Public type surface for `spawn/*` modules. Most of this is straight
 *   Node.js child_process types duplicated so the lib doesn't carry a hard
 *   `@types/node` requirement at the surface. The `Spawn*` types layer
 *   Socket-specific options on top:
 *
 *   - `SpawnOptions.spinner` lets the caller pause an outer spinner while the
 *     child runs (so progress output isn't smeared by the spinner's redraw
 *     loop).
 *   - `SpawnOptions.stripAnsi` defaults to true — most consumers want plain text
 *     from captured output. Pass false only when the output is being forwarded
 *     to a terminal directly.
 *   - `SpawnOptions.stdioString` defaults to true — string output is ergonomic;
 *     opt out for binary streams. `PromiseSpawnResult` is the upstream
 *     `@npmcli/promise-spawn` shape: a Promise enriched with `process` +
 *     `stdin` accessors so callers can reach into the running child without
 *     re-spawning.
 */
 ...
 *
 * @property {string | undefined} cwd - Current working directory for the
 *   process.
 * @property {NodeJS.ProcessEnv | undefined} env - Environment variables for the
 *   process.
 ...
 * @property {StdioType | undefined} stdio - Stdio configuration (`'pipe'`,
 *   `'ignore'`, `'inherit'`, or array)
 * @property {boolean | undefined} stdioString - Convert stdio output to strings
 *   (default: `true`)
 * @property {number | undefined} timeout - Maximum time in milliseconds before
 *   killing the process.
 * @property {number | undefined} uid - User identity of the process (POSIX
 *   only)
 * @property {boolean | undefined} windowsVerbatimArguments - Don't quote or
 *   escape arguments on Windows (requires shell: true). Use when you need exact
...
/**
 * Result returned by {@link spawn} when the child process completes. This is a
 * Promise that resolves with process exit information and output, with
 * additional properties for accessing the running process and stdin stream.
 *
 * @example
 ...
 * @property {WritableStreamType | null} stdin - Writable stream for process
 *   stdin, or `null` if not piped.
 */
...
/**
 * Error object thrown when a spawned process fails. Extends the standard Error
 * with process-specific information including exit code, signal, command
 * details, and captured output.
 *
 * @example
 *   try {
 *     await spawn('exit', ['1'])
...
/**
 * Extra options passed to the underlying promise-spawn implementation. This is
 * an open-ended object for passing additional metadata or configuration.
 */
export type SpawnExtra = Record<string, unknown>

/**
 * Valid values for individual stdio streams. - `'pipe'` - Creates a pipe
 * between child and parent (default) - `'ignore'` - Ignores the stream -
 * `'inherit'` - Uses parent's stream - `'overlapped'` - Windows-specific
 * overlapped I/O.
 */
export type IOType = 'pipe' | 'ignore' | 'inherit' | 'overlapped'

/**
 * Configuration for the process stdin, stdout, and stderr streams. Can be a
 ...
 * synchronously.
 *
 * @template T - Type of stdout/stderr, either string or Buffer.
 *
...
/**
 * Options for spawning a child process with {@link spawn}. Extends Node.js spawn
 * options with additional Socket-specific functionality.
 *
 ...
 *
 * @property {string} cmd - Command that was executed.
 * @property {string[] | readonly string[]} args - Arguments passed to the
 *   command.
 * @property {number} code - Process exit code.
 * @property {NodeJS.Signals | null} signal - Signal that terminated the
 *   process, if any.
 * @property {string | Buffer} stdout - Standard output (string if `stdioString:
 *   true`, Buffer otherwise)
 * @property {string | Buffer} stderr - Standard error (string if `stdioString:
 *   true`, Buffer otherwise)
 */
