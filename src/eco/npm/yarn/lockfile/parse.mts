 ...
 *   and additionally honors `linkType: soft` to skip workspace links and
 *   `dependenciesMeta:` blocks that can mark deps optional. The parser is
 ...
 *      line walker, since yarn's syml-as-it-uses-it is a strict subset.
...
export function consumeDependenciesMeta(
  ...
  // which are very common, show up as optional. Consume the block to
  // advance `pos`, but don't synthesize anything from it. The parent's
  // own optionality comes from how upstream references it, which yarn
...
export function consumeEntryProperties(
  ...
  while (pos < content.length) {
    ...
    if (hasYarnPropertyKey(propLine, 'version')) {
      ...
      setYarnResolution(entry, propLine)
...
export function freezeYarnEntry(entry: YarnEntryState): PackageRef {
...
export function hasYarnPropertyKey(line: string, key: string): boolean {
  return (
    StringPrototypeIndexOf(line, `${key} `) === 0 ||
    StringPrototypeIndexOf(line, `${key}:`) === 0
  )
}

export function isYarnIgnoredLine(line: string): boolean {
...
export function jsParseYarnLock(content: string): ParsedLockfile {
  ...
  while (pos < content.length) {
    ...
    if (entry.name && entry.version) {
      const ref = freezeYarnEntry(entry)
...
export function setYarnResolution(
  entry: YarnEntryState,
  propLine: string,
): void {
  const colonIdx = StringPrototypeIndexOf(propLine, ':')
...
export function valueAfterKey(line: string, keyLen: number): string {
  ...
  // length of Classic's space-terminated `'key '`. Berry's `key:`
