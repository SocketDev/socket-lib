/**
 * @fileoverview Bump the max-listener cap on an EventTarget (or
 * AbortSignal). Lives next to the suppress-warnings family because
 * the goal is the same — silence Node's noise about exceeding the
 * default 10-listener threshold — but the mechanism is different:
 * here we set the symbol on the target itself, no
