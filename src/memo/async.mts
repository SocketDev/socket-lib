...
import { PromiseResolve } from '../primordials/promise.mjs'

import { cacheRegistry, defaultKeyGen } from './shared.mjs'

import type { CacheEntry, MemoizeOptions } from './types.mjs'
...
export function memoizeAsync<Args extends unknown[], Result>(
  ...
  return async function memoized(...args: Args): Promise<Result> {
    ...
    // Create promise and cache it immediately so calls deduplicate.
